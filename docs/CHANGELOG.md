...
An "On the site now" panel on the publish screen — live listings, taken off by
the sold check, held at review, hidden for other reasons — shown whether or not
an upload is in progress. Plus what came off week by week, and a CSV download of
every live listing including the hidden ones.
...
Same code as 1.57, renumbered and re-cut as complete repositories.

---

## v1.57 — 2026-09-08

**Backend fixes found by testing against Postgres**

Two faults that were already live and that SQLite could never have shown. Frontend
unchanged apart from the build number. Detail in the backend `CHANGELOG.md`.

---

## v1.56 — 2026-09-08

**"Archived" was the wrong word**

Read against a 9,281-row load sitting under a 672-row one, "archived" says those
9,281 houses were archived. A superseded FILE is not superseded LISTINGS —
everything still advertised is carried into the current load, and a listing
leaves only when the daily link check finds its advertisement gone. The import
history now says **superseded**, with a line saying what happened to the houses.
...
Three endpoints were reachable by anyone with no login — the full API
documentation, a self-test that printed real addresses and valuations, and a
readiness probe that named the database tables. All closed. Frontend unchanged
apart from the build number, which is bumped in lockstep. Detail in the backend
...
Eleven loose files at the repository root, now nine. The two documents moved to
`docs/`; the nine that remain are all required at the root by Next.js, npm,
TypeScript, Tailwind and Playwright, and moving any of them breaks the build.
Nothing deleted, no application code moved.
...
A new button on the properties page, beside the CSV export: **Live data +
checks (Excel)**. The CSV stays as the raw dump; this is the one you open when
you want to know whether the data is right. It leads with a sheet of checks —
each a question, the answer beside it, and what the answer should look like —
and includes the listings a customer cannot see, which is where a fault hides.
...
The publish screen now says, in plain numbers, when an earlier upload was
smaller than the book and archived the rest — and offers to put them back. It
appears in both states of that page, because after a publish there is nothing
staged, and that is exactly when a lost book shows up. Full detail in the
backend repository's `CHANGELOG.md`.
...
A backend fix — the frontend is unchanged apart from the build number, which is
bumped in lockstep so the side-by-side check on the admin dashboard stays
meaningful. Full detail in the backend repository's `CHANGELOG.md`.

---

## v1.50 — 2026-09-07

**Photographs and links come from our own domain now**

The backend encrypts every source URL and serves the picture itself, so nothing
the browser receives names a supplier. This repo holds the other half:

- `hiRes` could no longer see a size token to rewrite, because the real URL
  never reaches the browser. Left alone it would have returned the proxy path
  untouched and every photograph on the site would have quietly fallen back to
  the ~300px thumbnail the scrape stored. It asks for the width it wants and
  the proxy applies it to the real address.
- The response keys stopped carrying the supplier's name. The number is the
  same number; reading the field no longer tells you where we bought it.
- `next.config.js` lists no remote image hosts. That file is fetched by the
  browser, so naming them there was the leak in the first place.

**A note on the numbering below.** Entries older than this one are from the 9.x
line, before the version restart; the full build log lives in the backend
repository's `CHANGELOG.md`, which is the one the admin dashboard reads against.

---

## v4.1 — 2026-08-17

**The repeating 401s: the badge poll now stops instead of retrying forever**

The 401s in the Railway log are all one request on a timer — the wish-list badge
count in the header, once every sixty seconds, with no valid token behind it:

```
19:37:08  GET /api/wishlists/notifications  401
19:38:08  GET /api/wishlists/notifications  401
19:39:08  GET /api/wishlists/notifications  401
```

Nothing is broken by them. But they are partly my doing, and they should not be
there.

v3.5 made that poll `background` so its failures could no longer navigate —
correct, because an expired token was throwing people to the sign-in page
mid-sentence. What it did not do was stop the poll from trying again. So the
behaviour went from *one 401 and a bounce* to *a 401 every minute, forever*: a
quieter fault, and a longer-lived one.

The poll now stops on 401 or 402. Neither is going to resolve itself on a timer,
and a request that cannot succeed should not keep being made — it is noise in
the log for us and wasted battery for the person holding the phone.

Sessions being 12 hours since v3.5 already makes this far rarer; this closes the
case where it happens anyway.
