...
import { streetBasemap } from "@/lib/basemap";
  ...
  useEffect(() => {
    ...
    loadLeaflet()
      .then(async (L) => {
        ...
        mapRef.current = map;
        const base = await streetBasemap();
        // maxNativeZoom comes from the source: Esri's grey canvas has real
        // pixels only to 16 and LINZ has them to 22. Claiming more than a
        // source has asks for tiles that do not exist and paints grey squares
        // at close zoom.
        streetRef.current = L.tileLayer(base.url, {
          attribution: base.attribution,
          maxNativeZoom: base.maxNativeZoom,
          maxZoom: 22,
        }).addTo(map);
        if (base.labels) {
          L.tileLayer(base.labels, {
            maxNativeZoom: base.maxNativeZoom, maxZoom: 22,
          }).addTo(map);
        }
