...
const COLS: Col[] = [
  ...
  // Beside the price, not buried at the end. This is the column whose absence
  // made the batch unauditable: 1,128 auctions were published as fixed-price
  // asks and nothing on this screen contradicted it — "Price basis" one column
  // over actually confirmed it, reading "advertised".
  //
  // The feed's own words, then what we concluded from them, so a reviewer can
  // see the two disagree. A grid that shows only the conclusion is the model
  // marking its own work.
  { key: "sale_method", label: "How it sells", kind: "text", hint: "the feed's own word" },
  { key: "listing_type", label: "Read as", kind: "text", hint: "what we concluded" },
  ...
  // A blank Margin and a blank "could not price it" look identical in a
  // spreadsheet, and they are opposite problems. This says which rule withheld
  // the margin, in words, so a full export answers "why is this not a deal"
  // without anybody opening the pricing code.
  { key: "deal_block_reason", label: "Why no deal", kind: "text" },
  // A house that had a price last week and now says by-negotiation carries
  // last week's price forward, less a negotiation discount. In the Asking
  // column that is indistinguishable from a price a vendor named this week,
  // which is exactly the distinction somebody reviewing a batch needs.
  { key: "asking_basis", label: "Price basis", kind: "text" },
  { key: "prior_asking_price", label: "Was listed at", kind: "money" },
  { key: "prior_asking_seen_at", label: "Seen at that price", kind: "date" },
];
...
export default function StagedReviewGrid({ region = "Auckland" }: { region?: string }) {
  ...
  // Keep a row out of the live site without taking it out of the batch. The
  // other half of Remove: this one is real, it just should not be in front of
  // customers yet, and it stays in the totals and can be released later.
  async function rowHold(id: number) {
    setRowBusy(id); setRowMsg({ id, text: "Hiding…" });
    try {
      await api(`/api/admin/listings/${id}/hold?reason=${encodeURIComponent("Hidden by admin")}`, { method: "POST" });
      setRowMsg({ id, text: "Hidden — it will not go live" });
      setReloadKey((k) => k + 1);
    } catch (e: any) { setRowMsg({ id, text: `Hide failed: ${e?.detail || e?.message || "error"}` }); }
    finally { setRowBusy(null); }
  }

  // Take a row out of the batch. Holding and removing answer different
  // questions: a held listing is real but not for the site, and stays in the
  // totals; a removed one is not a house and should not be in the numbers at
  // all. Confirmed because it cannot be undone from here — re-loading the file
  // brings it back, which is the only way back.
  async function rowRemove(id: number, address: string | null) {
    if (!window.confirm(
      `Remove ${address || "this listing"} from the batch?\n\n` +
      "It will be taken out of the totals and will not go live. " +
      "Loading the file again brings it back."
    )) return;
    setRowBusy(id); setRowMsg({ id, text: "Removing…" });
    try {
      await api(`/api/admin/listings/${id}`, { method: "DELETE" });
      setRowMsg({ id, text: "Removed from the batch" });
      setReloadKey((k) => k + 1);
    } catch (e: any) { setRowMsg({ id, text: `Remove failed: ${e?.detail || e?.message || "error"}` }); }
    finally { setRowBusy(null); }
  }
                ...
                <tr key={r.id} className={`border-b border-line/60 hover:bg-[#FAFAFA] ${r.is_held ? "bg-[#FEF6F4]" : ""}`}>
                  ...
                  <td className="px-3 py-1.5 whitespace-nowrap">
                    <div className="flex items-center gap-1.5">
                      ...
                      {!r.is_held && (
                        <button
                          onClick={() => rowHold(r.id)}
                          disabled={rowBusy === r.id}
                          className="px-2 py-1 text-[11px] border border-line rounded hover:border-amber-600 hover:text-amber-700 disabled:opacity-40"
                          title="Keep this listing out of the live site — it stays in the batch and can be released later"
                        >Hide</button>
                      )}
                      {r.is_held && (
                        <button
                      ...
                      <button
                        onClick={() => rowRemove(r.id, r.address)}
                        disabled={rowBusy === r.id}
                        className="px-2 py-1 text-[11px] border border-line rounded hover:border-red-600 hover:text-red-600 disabled:opacity-40"
