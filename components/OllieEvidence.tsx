...
export default function OllieEvidence({ answer, onAsk, disabled = false }: { answer: string; onAsk?: (question: string, label?: string) => void; disabled?: boolean }) {
  ...
  return <section aria-label="Property facts from Apex records" style={{ border:"1px solid #D9E3DD",borderRadius:16,padding:16,background:"#FFFFFF",color:"#202D29" }}>
    <h3 style={{margin:"0 0 6px",fontSize:18}}>Compare the recorded facts</h3>
    <p style={{fontSize:12,color:"#52645C",margin:"0 0 14px"}}>{checked ? `${loaded} of ${rows.length} linked records loaded. Checked at ${checked}.` : 'Checking linked Apex property records.'} This check time is not the age of the listing. Up to four linked properties.</p>
    ...
    {rows.map(({id,property:p}) => {
      ...
      return <article key={id} style={{background:"#F7F8F5",padding:14,borderRadius:12,border:"1px solid #D9E3DD"}}>
        <Link href={`/property/${id}`} style={{fontSize:16,fontWeight:700,color:"#176B57"}}>{p.address || `Property ${id}`}</Link>
        <p style={{margin:"5px 0 12px",fontSize:13,color:"#52645C"}}>{p.suburb || 'Suburb not recorded'}{p.off_market ? ' · Off market' : ''}</p>
        <dl style={{margin:0}}>{facts.map(([name,value])=><div key={name} style={{display:"flex",justifyContent:"space-between",gap:12,fontSize:13,padding:"5px 0"}}><dt style={{color:"#52645C"}}>{name}</dt><dd style={{margin:0,textAlign:"right"}}>{value}</dd></div>)}</dl>
        {p.asking_basis && p.asking_basis !== 'advertised' && <p style={{fontSize:12}}>Price basis: {p.asking_basis}</p>}
        {comparison && <p style={{fontSize:13,borderTop:"1px solid #D9E3DD",paddingTop:10}}>{comparison.discount >= 0 ? `${comparison.discount.toFixed(1)}% below` : `${Math.abs(comparison.discount).toFixed(1)}% above`} Apex estimate · {comparison.uplift.toFixed(1)}% value uplift over asking.</p>}
        <p style={{fontSize:12,color:"#52645C"}}>Condition and reasons for pricing are not verified here. An estimated value gap is not profit.</p>
        <Link href={`/property/${id}`} style={{fontSize:13,color:"#176B57"}}>View property and sold evidence →</Link>
      </article>;
    })}
    </div>
    </details>
