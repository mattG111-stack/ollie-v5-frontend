...
export default function SubdivisionCalc({
  ...
  return (
    <Card>
      ...
      {/* Reality check — the modelled sections are worth far more than the site
          itself, so these figures aren't credible. Shown, not hidden, so the user
          sees why it isn't counted as an opportunity. */}
      {data.implausible_vs_value && (
        <div
          style={{
            marginTop: 12,
            background: "rgba(212,80,62,.08)",
            border: "1px solid rgba(212,80,62,.28)",
            borderRadius: 12,
            padding: "12px 14px",
            fontSize: 13,
            color: C.danger,
        ...
        >
          <strong>{t("subcalc.implausibleTitle")}</strong> {t("subcalc.implausibleBody")}
        </div>
      )}

      {/* Feasibility summary — the design's field set */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 40px", marginTop: 12 }}>
        <div>
      ...
      <div style={{ marginTop: 20, borderTop: `1px solid ${C.divider}`, paddingTop: 4 }}>
        ...
        />
        <Row label={t("subcalc.grossSales")} value={money(data.gross_sales)} strong />
        <Row label={t("subcalc.lessPurchase")} value={money(data.buy_price != null ? -data.buy_price : null)} />
        <Row
          label={t("subcalc.lessConsent")}
        ...
        {!!data.incidentals_cost && (
          <Row label={t("subcalc.lessIncidentals")} value={money(-data.incidentals_cost)} />
        )}
        {!!data.demolition_cost && (
          <Row label={t("subcalc.lessDemolition")} value={money(-data.demolition_cost)} />
        )}
        {!!data.holding_cost && (
          <Row label={t("subcalc.lessHolding", { years: data.holding_years ?? 2 })} value={money(-data.holding_cost)} />
        )}
        {!!data.contingency_cost && (
          <Row label={t("subcalc.lessContingency")} value={money(-data.contingency_cost)} />
        )}
        {!!data.gst_cost && (
          <Row label={t("subcalc.lessGst")} value={money(-data.gst_cost)} />
        )}
        <Row
      ...
      <details style={{ marginTop: 18, borderTop: `1px solid ${C.divider}`, paddingTop: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 40px", marginTop: 10 }}>
...
function Row({
  ...
  return (
    ...
    >
      ...
      <span
        className="tnum"
        style={{ fontSize: 15, fontWeight: strong ? 700 : 600, textAlign: "right", color }}
      >
        {value}
      </span>
    </div>
