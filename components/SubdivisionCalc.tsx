 ...
 * batch ingest computes.
 *
 * Edits are per-session and never persisted — WITH ONE EXCEPTION. In preview,
 * the lot count is not a what-if: it is a correction being made to a listing
 * before it goes live, so it is saved to the row, and it survives the re-price
 * and the publish. Everything else here, and everything on the live site, stays
 * a scratchpad — a customer changing their own copy of the numbers must not
 * write anything.
 */
...
type Scenario = {
  ...
  /** The plan NOT chosen, and what it would have earned. Null when there was no
   *  second option — a bare site has no house to keep or knock down. */
  alternative_strategy?: string | null;
  alternative_profit?: number | null;
  /** True when the winning plan knocks the house down. */
  demolish?: boolean;
  is_terrace: boolean;
...
const LEFT: Field[] = [
  ...
  // "lots" is deliberately NOT here. It is edited on the row that displays it,
  // in additional lots. Listing it here too would put two controls on one
  // number, counting in two different units — which is how it got lost the
  // first time.
...
export default function SubdivisionCalc({
  ...
  // In PREVIEW the lot count is not a what-if — it is a correction being made
  // to a listing before it goes live, so it is saved to the row and survives
  // the re-price and the publish. On the live site the same control stays a
  // scratchpad: a customer changing their own copy of the numbers must not
  // write anything.
  const [canSave, setCanSave] = useState(false);
  const [saved, setSaved] = useState<"saving" | "saved" | "failed" | null>(null);
  useEffect(() => setCanSave(isPreview()), []);

  // total lots, or null to hand the site back to the model
  const saveLots = useCallback(async (total: number | null) => {
    if (!canSave) return;
    setSaved("saving");
    try {
      await api(`/api/admin/listings/${propertyId}/lots`, {
        method: "POST", body: JSON.stringify({ lots: total }),
      });
      setSaved("saved");
    } catch {
      setSaved("failed");
  ...
  const assemble = useCallback((m: "retain" | "full", next: Partial<Record<FieldKey, string>>) => {
    ...
    // The lot count is edited on the row that displays it rather than in the
    // field list, so it has to be added here. FIELDS is what this loop walks,
    // and anything not in it is silently never sent — which would leave a
    // control on screen that changes nothing.
    const rawLots = next.lots;
    if (rawLots !== undefined && rawLots !== "") {
      const v = parseNum(rawLots, false);
      if (v !== null && v !== undefined) b.lots = v;
    }
    return b;
  }, []);
    ...
    <Card>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(230px,1fr))", gap: "0 40px", marginTop: 12 }}>
        <div>
          ...
          {/* EDITABLE, in place, on the number itself.
              It was editable before this — as "Lots the site takes", inside the
              collapsed "Adjust the numbers" panel, counting TOTAL lots while
              this row shows ADDITIONAL ones. So the figure you were looking at
              and the control that changed it were in different places under
              different names counting different things, which is the same as
              not being editable at all.
              One control now, on the row that shows the number, in the units
              that row is written in. */}
          <LotsRow
            addl={data.max_addl_lots}
            edited={edits.lots !== undefined}
            busy={busy}
            saved={canSave ? saved : null}
            onChange={(nextAddl) => {
              // The model is asked for TOTAL lots; this row is written in
              // additional ones. The retained house is the difference.
              const total = nextAddl === null ? null : nextAddl + 1;
              onEdit("lots", total === null ? "" : String(total));
              void saveLots(total);
            }}
            onReset={() => { onEdit("lots", ""); void saveLots(null); }}
            t={t}
          />
        </div>
        <div>
          ...
          <Row
            ...
            last
          />
          {/* The road not taken. Telling somebody to demolish a house is a big
              thing to say, and a headline on its own is an instruction; the
              number it beat is what makes it an argument. Shown both ways
              round — knowing that keeping the house is $51k behind is as
              useful as knowing demolishing it is $51k ahead. */}
          {data.alternative_strategy != null && data.alternative_profit != null
            && profit != null && (
            <div
              style={{
                marginTop: 10, paddingTop: 10,
                borderTop: `1px solid ${C.divider}`,
                fontSize: 12.5, color: C.label, lineHeight: 1.55,
              }}
            >
              {data.demolish ? t("subcalc.altKeeping") : t("subcalc.altDemolishing")}{" "}
              <span style={{ fontFamily: MONO, color: C.text }}>
                {money(data.alternative_profit)}
              </span>
              {" — "}
              <span style={{ color: data.alternative_profit < profit ? C.label : C.text }}>
                {t("subcalc.altBy", { amount: money(Math.abs(profit - data.alternative_profit)) })}
              </span>
            </div>
          )}
        </div>
      ...
      <div style={{ marginTop: 20, borderTop: `1px solid ${C.divider}`, paddingTop: 4 }}>
        ...
        <Row
          ...
          strong
        />
        <Row label={t("subcalc.lessPurchase")} value={money(data.buy_price != null ? -data.buy_price : null)} caption="buying the site — our estimated buy price" />
        ...
        <Row
          ...
          caption={
            // The split is only real when there is a total to split. With no
            // services cost — a site we could not put a purchase figure to —
            // this subtracted the consent portion from zero and printed
            // "build −$1,000,000 + consent/civils $1,000,000" against a $0
            // line: three invented numbers in one caption, on a panel where
            // every other figure was an honest dash.
            data.is_terrace && (data.services_cost ?? 0) > 0
              ...
              : data.is_terrace
              ? "build cost + consent and civils per terrace"
...
/** The lot count, editable where it is shown.
 *
 * The model works out how many lots a site takes from its area and the zone's
 * minimum. On a site that has already been worked — a wide frontage, a corner,
 * a rear access strip — it takes the cautious reading, so the profit shown is a
 * floor rather than an estimate. This is how you say what the site really does.
 *
 * Counted in ADDITIONAL lots, because that is what the row has always shown.
 * The server is asked for the total; the conversion happens at the call site so
 * there is exactly one place where the two meanings meet.
 */
function LotsRow({
  addl, edited, busy, saved, onChange, onReset, t,
}: {
  addl: number | null;
  edited: boolean;
  busy: boolean;
  saved: "saving" | "saved" | "failed" | null;
  onChange: (addl: number | null) => void;
  onReset: () => void;
  ...
  const step = (by: number) => {
    const next = n + by;
    if (next < 0 || next > 40) return;      // matches the model's own ceiling
    onChange(next);
  };
  const btn: React.CSSProperties = {
    width: 30, height: 30, borderRadius: 8, cursor: "pointer",
    border: `1px solid ${C.border}`, background: "#fff",
    fontSize: 17, lineHeight: 1, color: C.text, fontWeight: 700,
  };
  return (
    <div style={{ padding: "13px 0" }}>
      <div style={{ display: "flex", justifyContent: "space-between",
                    alignItems: "center", gap: 12 }}>
        <span style={{ fontSize: 15, color: C.label }}>{t("subcalc.additionalLots")}</span>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <button type="button" style={btn} onClick={() => step(-1)}
                  aria-label={t("subcalc.lotsFewer")} disabled={busy || n <= 0}>−</button>
          <input
            type="number" min={0} max={40} value={n}
            onChange={(e) => {
              const v = e.target.value;
              onChange(v === "" ? null : Math.max(0, Math.min(40, Number(v))));
            }}
            style={{
              width: 62, textAlign: "center", fontSize: 16, fontWeight: 700,
              color: C.text, padding: "5px 6px", borderRadius: 8,
              border: `1px solid ${edited ? C.text : C.border}`, background: "#fff",
            }}
          />
          <button type="button" style={btn} onClick={() => step(1)}
                  aria-label={t("subcalc.lotsMore")} disabled={busy}>+</button>
        </div>
      </div>
      <div style={{ fontSize: 11.5, color: "#8A94A6", marginTop: 3, lineHeight: 1.35 }}>
        {saved ? (
          // In preview this is a correction, so it has to say whether it stuck.
          // A number that silently failed to save is worse than no control at
          // all: the review looks done and the wrong figure goes live.
          <span style={{ color: saved === "failed" ? C.danger : undefined }}>
            {t(saved === "saving" ? "subcalc.lotsSaving"
               : saved === "saved" ? "subcalc.lotsSaved"
               : "subcalc.lotsSaveFailed")}
          </span>
        ) : edited ? (
          <>
            {t("subcalc.lotsEdited")}{" "}
            <button type="button" onClick={onReset}
                    style={{ background: "none", border: "none", padding: 0,
                             color: C.text, textDecoration: "underline",
