...
const adminNav: Nav[] = [
  ...
  { href: "/admin/customers", label: "nav.whoTheyAre" },
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  // The rail folds away on a desktop, with a tab on the edge to bring it back.
  // Remembered, because a person who wants the width back wants it back on
  // every page, not once. Starts open — a collapsed nav on a first visit is a
  // product that hides its own map.
  const [railOpen, setRailOpen] = useState(true);
  useEffect(() => {
    try {
      setRailOpen(localStorage.getItem("apex:rail") !== "closed");
    } catch { /* private mode: keep the default */ }
  }, []);

  // Ollie's page has no header and paints itself in the app's dark. It is a
  // room, not a list, so the rail folds away on arrival without being asked.
  const onOllie = pathname === "/ask";

  /**
   * A per-visit override of the automatic fold.
   *
   * Ollie collapses the rail for you, but pulling it back out while you are
   * there has to stick — a tab that springs shut again is a broken control.
   * Held separately from the remembered preference and cleared on navigation,
   * so opening it on Ollie does not quietly change what every other page does,
   * and coming back to Ollie later still opens clear.
   */
  const [railOverride, setRailOverride] = useState<boolean | null>(null);
  useEffect(() => { setRailOverride(null); }, [pathname]);

  const railVisible = onOllie ? railOverride ?? false : railOpen;

  function toggleRail(open: boolean) {
    if (onOllie) { setRailOverride(open); return; }
    setRailOpen(open);
    try { localStorage.setItem("apex:rail", open ? "open" : "closed"); } catch { /* ignore */ }
  }

  // Which features get used, and for how long. Sits here so it covers every
  // signed-in page without each one having to remember to opt in.
  usePageTracking();
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", width: "100%", background: C.page }}>
      ...
      <aside
        style={{
          ...
          ...(isMobile
            ...
            : {
                flex: railVisible ? "0 0 264px" : "0 0 0px",
                width: railVisible ? 264 : 0,
                padding: railVisible ? "26px 20px" : "26px 0",
                overflow: "hidden",
                position: "sticky",
                transition: "flex-basis .28s ease, width .28s ease, padding .28s ease",
              }),
        }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "0 4px 4px" }}>
          <Link href="/today" style={{ display: "flex", alignItems: "center", gap: 12, color: "#EFE9DA", flex: 1, minWidth: 0 }}>
          ...
          {!isMobile && (
            <button
              ...
              style={{
                ...
                color: "#9EA6B8", cursor: "pointer", display: "flex",
                alignItems: "center", justifyContent: "center",
              }}
            >
              <svg width="13" height="13" viewBox="0 0 14 14" fill="none" aria-hidden>
                <path d="M9 2.5L4.5 7 9 11.5" stroke="currentColor" strokeWidth="1.8"
      ...
      {/* The way back. Sits on the very edge, vertically centred, so it is
          findable without taking a column of the page to say so. */}
      {!isMobile && !railVisible && (
        <button
          ...
          style={{
            ...
            zIndex: 40, width: 24, height: 74,
            borderRadius: "0 10px 10px 0",
            border: "1px solid rgba(255,255,255,.12)", borderLeft: "none",
            background: C.dark, color: "#9EA6B8", cursor: "pointer",
            display: "flex", alignItems: "center", justifyContent: "center",
            boxShadow: "0 8px 26px rgba(0,0,0,.4)",
          }}
        >
          <svg width="13" height="13" viewBox="0 0 14 14" fill="none" aria-hidden>
            <path d="M5 2.5L9.5 7 5 11.5" stroke="currentColor" strokeWidth="1.8"
                  strokeLinecap="round" strokeLinejoin="round" />
            <path d="M2 2v10" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
          </svg>
        </button>
      )}

      <main style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
        {/* Ollie's page carries no header. A search bar over the top of him is
            a second input asking to be typed in, and the rail already holds
            settings, sign-out and every route — nothing is stranded. On a
            phone the header IS the only way to the menu, so it stays. */}
        {(!onOllie || isMobile) && (
          <TopBar initials={initials} onMenu={() => setNavOpen(true)} showMenu={isMobile}
                  pollMatches={!isPromoter} bare={onOllie} />
        )}
        {children}
      </main>
    </div>
...
function TopBar({ initials, onMenu, showMenu, pollMatches = true, bare = false }: {
  initials: string; onMenu?: () => void; showMenu?: boolean;
  /** Ollie's page has no header on a desktop at all. On a phone the menu
   *  button is the only way into the nav, so a stripped one survives there —
   *  the button and the avatar, in his colours, and nothing else. */
  bare?: boolean;
  ...
  return (
    <header
      style={{
        ...
        borderBottom: `1px solid ${bare ? "rgba(126,154,192,.12)" : C.border}`,
    ...
    >
      {showMenu && (
        <button
          ...
          style={{
            ...
            border: `1px solid ${edge}`, background: surface, cursor: "pointer",
      ...
      {/* No search on Ollie. He has his own question box, and two input bars on
          one screen is a question about which to type in. */}
      <div ref={searchRef} hidden={bare} style={{ position: "relative", flex: 1, minWidth: 0 }}>
      <form
        onSubmit={submit}
      ...
      {open && sugg.length > 0 && (
        <div
          ...
          style={{
            ...
            background: surface, border: `1px solid ${edge}`, borderRadius: 12,
          ...
          {sugg.map((s, i) => (
            ...
            >
              ...
              <span style={{ minWidth: 0, overflow: "hidden" }}>
                <span style={{
                  display: "block", fontSize: 14, color: text, fontWeight: 600,
