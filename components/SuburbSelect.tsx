...
export function loadSuburbs(region = "Auckland", dataset: SuburbDataset = "any"): Promise<SuburbOption[]> {
  const key = `${region}|${dataset}`;
  if (!CACHE[key]) {
    CACHE[key] = api<SuburbOption[]>(
      `/api/properties/suburbs?region=${encodeURIComponent(region)}&dataset=${dataset}`,
    ).catch(() => {
...
export function useSuburbs(region = "Auckland", dataset: SuburbDataset = "any") {
  const [options, setOptions] = useState<SuburbOption[]>([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    let alive = true;
    loadSuburbs(region, dataset)
...
export default function SuburbSelect({
  ...
  const { options, loading } = useSuburbs(region, dataset);
