 ...
 */
/**
 * Which set of suburbs to offer.
 *
 * "for_sale" is the one the properties page needs. The sold archive covers far
 * more suburbs than any single week of listings, so a merged list hands that
 * page dozens of suburbs with nothing live in them — pick one and the screen
 * goes blank, which is indistinguishable from a filter that does not work. That
 * is exactly what it looked like.
 *
 * The trends panel wants "any", because it reads sold data and a suburb with no
 * live listings still has years of sales to chart.
 */
export type SuburbDataset = "any" | "for_sale" | "sold";

const CACHE: Record<string, Promise<SuburbOption[]>> = {};
export function loadSuburbs(
  region = "Auckland",
  dataset: SuburbDataset = "any",
  district = "",
): Promise<SuburbOption[]> {
  const key = `${region}|${dataset}|${district}`;
  if (!CACHE[key]) {
    const d = district ? `&district=${encodeURIComponent(district)}` : "";
    CACHE[key] = api<SuburbOption[]>(
      `/api/properties/suburbs?region=${encodeURIComponent(region)}&dataset=${dataset}${d}`,
    ).catch(() => {
      delete CACHE[key];     // let the next mount retry rather than caching a failure
...
export function useSuburbs(region = "Auckland", dataset: SuburbDataset = "any", district = "") {
  const [options, setOptions] = useState<SuburbOption[]>([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    let alive = true;
    setLoading(true);
    loadSuburbs(region, dataset, district)
      .then((r) => { if (alive) setOptions(r); })
      .finally(() => { if (alive) setLoading(false); });
    return () => { alive = false; };
  }, [region, dataset, district]);
...
export default function SuburbSelect({
  ...
  region?: string;
  /** Offer only the suburbs the calling screen can actually show. */
  dataset?: SuburbDataset;
  /** When a district is chosen, offer only the suburbs inside it. Picking a
   *  district and then a suburb outside it is two filters that cannot both be
   *  satisfied — the screen empties and nothing says the two disagree. */
  district?: string;
  width?: number | string;
  ...
  const { options, loading } = useSuburbs(region, dataset, district);
