...
export default function SuburbTrendChart({ yearly, suburb, source }: Props) {
  ...
  const partialTail = last.partial;
  ...
  // A gradient id is global to the document, so two charts on one page both
  // painted with whichever was defined first — a falling suburb came out with a
  // rising suburb's green fill under a red line. Keyed by direction, the id is
  // shared only where the gradient is identical anyway.
  const fillId = `trendFill-${dir > 0 ? "up" : dir < 0 ? "down" : "flat"}`;
