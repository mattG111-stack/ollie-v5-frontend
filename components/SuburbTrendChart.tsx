...
/** A year the backend held back, and why — see parseWithheld. */
interface Withheld { year: number; sales: number; min: number }

/** The newest year is not always on the line: a year with a handful of sales
 *  is not a median, so it is left off. Ending the line early with no
 *  explanation reads as a chart that has stopped working — especially in a
 *  suburb the reader knows has been selling. */
function parseWithheld(raw: string | null | undefined): Withheld | null {
  if (!raw) return null;
  try {
    const w = JSON.parse(raw)?.withheld;
    if (w && Number(w.year)) {
      return { year: Number(w.year), sales: Number(w.sales || 0), min: Number(w.min || 0) };
    }
  } catch {}
  return null;
}
...
export default function SuburbTrendChart({ yearly, suburb, source }: Props) {
  const isMobile = useIsMobile();
  const { t } = useT();
  const yearlyData = useMemo(() => parseYearly(yearly), [yearly]);
  const withheld = useMemo(() => parseWithheld(yearly), [yearly]);
    ...
    <div className="bg-white border border-line rounded-card shadow-soft p-5 mt-5">
      ...
      {withheld && (
        <div className="text-[11px] text-faint mt-2">
          {withheld.year} is not on the line yet — {withheld.sales.toLocaleString()}{" "}
          {withheld.sales === 1 ? "sale" : "sales"} so far, and a year needs{" "}
          {withheld.min} before its median means anything.
        </div>
      )}
      ...
      <div className="text-[11px] text-faint mt-2">
        {!partialTail
          ...
          : throughMonth
            ...
            // No month from the backend (an older deploy): say the same thing
            // without claiming a date we were not told.
            : t("strend.ytdNoteNoMonth", {
                year: String(last.year),
                n: last.count.toLocaleString(),
              })}
      </div>
