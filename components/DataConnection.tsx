...
type CollectionRun = {id:number; kind:"for_sale"|"sold"; status:string; started_at:string|null; finished_at:string|null; observed:number; new?:number; pending?:number; discovery_pending?:number; excluded?:number};
...
export default function DataConnection() {
  ...
  return <section className="mt-8 border border-line rounded-xl p-5">
    ...
    {!!status?.latest_runs?.length && <ul className="text-sm mt-3 space-y-3">{status.latest_runs.map(run => <li key={run.id}>
      ...
      <span> · {run.status === "complete" ? "Batch completed" : run.status === "failed" ? "Attempt failed" : run.status === "running" ? "Last recorded as running" : "Status unknown"}</span>
      <div className="text-xs text-muted">{run.observed} source records observed{run.new != null ? ` · ${run.new} ${run.kind === "sold" ? "validated sales saved" : "new listings for review"}` : ""}{run.pending != null ? ` · ${run.pending} URLs queued` : ""}{run.discovery_pending ? " · Search discovery incomplete" : ""}</div>
