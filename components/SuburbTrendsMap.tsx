    ...
    (async () => {
      ...
      const add = (pts: Pt[], color: string, isSold: boolean) => {
        for (const p of pts) {
          ...
          // For-sale dots link through to the property page (sold have no page).
          const priceLine = `${isSold ? "Sold" : "Asking"} ${p.price ? fmtMoneyShort(p.price) : "—"}${p.sold_date ? ` · ${p.sold_date}` : ""}`;
          m.bindPopup(
            `<div style="min-width:150px"><b>${p.address ?? ""}</b><br/>${priceLine}` +
            (isSold ? "" : `<br/><a href="/property/${p.id}" style="display:inline-block;margin-top:6px;color:#2E353D;font-weight:700;font-size:12px;text-decoration:none">View property →</a>`) +
            `</div>`
          );
          group.addLayer(m);
      ...
      add(sale.points, "#1B2026", false); // near-black for-sale on top
      group.addTo(map);
    ...
    <div style={{ position: "relative" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
          <span style={{ width: 11, height: 11, borderRadius: "50%", background: "#1B2026", border: "2px solid #fff", boxShadow: "0 0 0 1px #cbd5e1" }} />
