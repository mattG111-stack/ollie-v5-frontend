...
export default function HeadlineDeals() {
  const { t } = useT();
  // The top-3 panels are admin only. The API already withholds the rows from
  // anyone else, so this is about not rendering two empty boxes rather than
  // about access — the gate that matters is the one on the server.
  const { me } = useAuth();
