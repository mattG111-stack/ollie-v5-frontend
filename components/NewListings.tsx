...
export default function NewListings() {
  ...
  async function fillGaps() {
    ...
    try {
      ...
      }>("/api/admin/release/listings/fill", { method: "POST" });
      setMsg(
        `${r.looked_up} looked up · ${r.fields_filled} details filled · ` +
        `${r.council_records_found} council records found` +
        (r.not_found ? ` · ${r.not_found} not on the council record` : "") +
        (r.unreachable ? ` · ${r.unreachable} never reached it` : ""),
      );
      await load();
    } catch (e: any) {
      setMsg(e?.detail || e?.message || "Could not fill those in");
    } finally {
      setFilling(false);
    }
  }
