...
export default function NewListings() {
  ...
  /* Fill the gaps a chunk at a time.
   *
   ...
   * no response body". It reads as a crash and is really a timeout; it is in
   * the bug log four times.
   *
   * Now each call does a small batch and says where to carry on from, and this
   * loops until the server reports nothing left to scan. Every request answers
   * in seconds, the running total updates as it goes, and a failure part-way
   * through keeps what was already filled instead of losing the lot. */
  async function fillGaps() {
    setFilling(true);
    let after = 0;
    let looked = 0, fields = 0, found = 0, missing = 0, unreachable = 0;
    // A ceiling on the loop. `scanned === 0` is the real stop condition; this
    // exists so that a server which somehow stops advancing cannot spin the
    // browser for ever.
    const MAX_ROUNDS = 200;
    try {
      for (let round = 0; round < MAX_ROUNDS; round++) {
        const r = await api<{
          looked_up: number; fields_filled: number; council_records_found: number;
          not_found: number; unreachable: number;
          last_id: number; remaining: number; scanned: number;
        }>(`/api/admin/release/listings/fill?kind=${tab}&after_id=${after}`,
          { method: "POST" });

        looked += r.looked_up;
        fields += r.fields_filled;
        found += r.council_records_found;
        missing += r.not_found;
        unreachable += r.unreachable;

        // Nothing scanned means we have walked past the last pending row.
        if (!r.scanned) break;
        // Guard against a server that returns rows but never advances the
        // cursor — without this the same chunk would be looked up on repeat.
        if (r.last_id <= after) break;
        after = r.last_id;

        setMsg(
          `${looked} looked up · ${fields} details filled · ` +
          `${found} council records found` +
          (r.remaining ? ` — ${r.remaining} to go…` : ""),
        );
      }
      setMsg(
        `${looked} looked up · ${fields} details filled · ` +
        `${found} council records found` +
        (missing ? ` · ${missing} not on the council record` : "") +
        (unreachable ? ` · ${unreachable} never reached it` : ""),
      );
      await load();
    } catch (e: any) {
      // Say what was done before it stopped. "Could not fill those in" after
      // three minutes of successful work is both wrong and infuriating.
      const done = looked
        ? ` (${fields} details filled on ${looked} listings before it stopped)`
        : "";
      setMsg((e?.detail || e?.message || "Could not fill those in") + done);
      await load();
    } finally {
      setFilling(false);
    }
