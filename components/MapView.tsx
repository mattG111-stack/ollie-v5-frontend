...
type MapPoint = {
  ...
  price?: number | null; listing_type?: string | null;
  est?: number | null; score?: number | null;
      ...
      {!loading && count === 0 && (
        <div className="absolute inset-0 z-[400] flex items-center justify-center pointer-events-none">
          <div className="bg-white border border-line rounded-xl px-5 py-4 text-center max-w-sm pointer-events-auto"
            ...
            <div className="font-semibold text-sm">No mapped listings</div>
            <div className="text-xs text-muted mt-1">
              No listings with a known location match these filters. Listings without
              coordinates may still appear in List view.
            </div>
          </div>
        </div>
      )}
...
function popupHtml(p: MapPoint, dataset: "for_sale" | "sold", t: (k: string, v?: any) => string) {
  ...
  // A for-sale pin with no advertised price says how the house sells instead.
  // "Asking: —" claims we do not know, and "Asking: Auction" is not a sentence —
  // so the label is dropped and the method stands on its own.
  const priceLine =
    dataset === "sold" || p.price != null
      ? `<div style="font-size:12px">${priceLabel}: <b>${money(p.price)}</b></div>`
      : `<div style="font-size:12px"><b>${escapeHtml(askingText(null, p.listing_type, t, true))}</b></div>`;
