...
// Brighter, higher-contrast than the app's muted palette on purpose — markers
// sit over a busy OSM basemap (parks, roads), so they need to pop.
const COLOR = {
  underpriced: "#2563EB",   // strong blue — no clash with the map's park greens
  subdividable: "#FF6A00",  // vivid orange
  forSale: "#3A4A63",       // slate for "other" (de-emphasised but still visible)
  sold: "#8894A6",
};
...
export default function MapView({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // CARTO Voyager: clean light basemap with soft colour — parks green, water
      // blue, legible streets (free, no API key, retina {r}). Same basemap as the
      // property detail-page map, so every map in the app reads consistently.
