...
import { askingText, fmtMoneyShort, fmtNumber, fmtPct } from "@/lib/format";
...
interface Props {
  ...
  /** Force a view, ignoring what the reader chose last. From ?view= only. */
  initialView?: ViewMode;
  /** Where to land when neither the URL nor the reader's history decides.
   *  All properties opens on the map; the deal-finder pages stay on the list. */
  defaultView?: ViewMode;
  /** Persist the reader's choice under this key and mirror it into ?view=.
   *  Omitted on embedded tables, which shouldn't fight the page for the URL. */
  viewMemoryKey?: string;
}
...
export default function PropertiesTable({
  ...
  // Where to open. An explicit ?view= wins; otherwise the reader's last choice,
  // read lazily so the server render and the first client render agree — reading
  // localStorage during render would hydrate "list" then flip to "map".
  const [view, setViewRaw] = useState<ViewMode>(initialView ?? defaultView);
  useEffect(() => {
    if (initialView || !viewMemoryKey) return;
    const saved = typeof window !== "undefined" ? localStorage.getItem(viewMemoryKey) : null;
    if (saved === "map" || saved === "list") setViewRaw(saved);
  }, [initialView, viewMemoryKey]);

  /** Switching view remembers the choice and puts it in the URL, so the page can
   *  be bookmarked, shared and reloaded on the map rather than snapping back. */
  const setView = (next: ViewMode) => {
    setViewRaw(next);
    if (!viewMemoryKey || typeof window === "undefined") return;
    localStorage.setItem(viewMemoryKey, next);
  ...
  function setFilter(k: string, v: string | null) {
    setFilters((prev) => {
      ...
      // Changing the district invalidates the suburb: a suburb chosen under the
      // old district is almost never inside the new one, and keeping it leaves
      // two filters that cannot both be satisfied. The result is an empty screen
      // with nothing to say the two disagree — which is exactly how this looked
      // like a broken suburb filter.
      if (k === "district" && next.suburb) delete next.suburb;
      return next;
    });
    setPage(1);
  }
      ...
      {showFilters && (
        <div className="flex flex-wrap gap-2 mb-3 items-center">
          ...
          <SuburbFilter
            ...
            onChange={(v) => setFilter("suburb", v || null)}
...
function PropertyRow({ row: r }: { row: ForSaleRow }) {
  ...
  const marginDollars =
    r.margin != null && r.fair_value != null && r.asking_price != null ? r.fair_value - r.asking_price : null;
  const marginColor =
  ...
  return (
    ...
    >
      ...
      <td className="px-3 py-2.5 text-right tabular-nums">{askingText(r.asking_price, r.listing_type, t, true)}</td>
