  ...
  return <section aria-label="Shortlist decision guide" style={{margin:"12px 0"}}>
    ...
    {insights.map(insight => <aside key={insight.kind} aria-label={insight.kind === "land_price_tradeoff" ? "Shortlist trade-off" : "Property insight"} style={{padding:"4px 0 16px",marginBottom:8,borderBottom:"1px solid #E5E7EB"}}>
      ...
      <details style={{fontSize:12,color:"#4E535A",margin:"12px 0"}}><summary style={{cursor:"pointer"}}>Evidence & limits · {insight.evidence.length} source values</summary>
        ...
        <ul style={{paddingLeft:18}}>{insight.evidence.map(e => <li key={`${e.propertyId}-${e.field}`} style={{margin:"8px 0"}}><a href={e.href} style={{color:"#454B54"}}>{e.address}</a>: {e.field === "asking_price" ? "asking price" : e.field === "fair_value" ? "Apex estimate" : "recorded land"} {e.unit === "NZD" ? money(e.value) : area(e.value)}</li>)}</ul>
        <ul style={{paddingLeft:18}}>{insight.limitations.map(limit => <li key={limit} style={{margin:"8px 0"}}>{limit}</li>)}</ul>
      </details>
      <a href={insight.nextAction.href} style={{display:"inline-block",color:"#454B54",fontSize:13,fontWeight:700}}>{insight.nextAction.label} →</a>
    </aside>)}
    <details open={insights.length === 0}><summary style={{cursor:"pointer",fontSize:14,color:"#454B54",padding:"10px 0",fontWeight:700}}>Compare prices & property details ({facts.length})</summary>
    <div style={{display:"flex",gap:16,flexWrap:"wrap",fontSize:12,marginBottom:14}}><span style={{color:"#454B54"}}>● Asking price</span><span style={{color:"#606D80"}}>● Apex estimate</span></div>
    {facts.map(({id,property:p,badges,checks}) => <article key={id} style={{padding:"20px 0",borderTop:"1px solid #DEDFE1"}}>
      ...
      <div aria-label={`Price comparison for ${p.address || id}`}>
        {([{label:"Asking price",value:p.asking_price,color:"#454B54"},{label:"Apex estimate",value:p.fair_value,color:"#606D80"}]).map(item=><div key={item.label} style={{margin:"8px 0"}}>
