 ...
 *
 * Renders whenever there's ANYTHING to show — even a listing with no recorded
 * sales still plots the current markers (our value / asking / sell estimate).
...
export default function SaleHistoryChart({
  sales, asking, askingDate, ourValue, expectedSale, expectedBand,
}: Props) {
  ...
  const layout = useMemo(() => {
    const hasSales = pts.length > 0;
    const nowVals = [asking, ourValue, expectedSale].filter((v): v is number => v != null && v > 0);
    // Nothing at all to plot → let the caller hide the card.
    if (!hasSales && !nowVals.length) return null;
    ...
    // X domain: real sales when we have them, else a short window ending today.
    const times = hasSales ? [...pts.map((p) => p.t), tNow] : [tNow - 4 * YEAR_MS, tNow];
    ...
    let linePts = "", areaD = "", lastX = xNow, lastY = 0;
    let saleLabels: { x: number; y: number; price: number }[] = [];
    let years: { x: number; label: string }[] = [];
    if (hasSales) {
      linePts = pts.map((p) => `${xOf(p.t).toFixed(1)},${yOf(p.price).toFixed(1)}`).join(" ");
      const first = pts[0], last = pts[pts.length - 1];
      lastX = xOf(last.t); lastY = yOf(last.price);
      areaD =
      ...
      years = pts.map((p) => ({ x: xOf(p.t), label: String(new Date(p.t).getFullYear()) }));
    }
    ...
    return {
      VB_W, x0, xNow, yTop, yBot, ticks, markers, cy, hasSales,
      linePts, areaD, bandRect, years, saleLabels,
      lastX, lastY, askY: asking ? yOf(asking) : null,
    };
  ...
  return (
    <div style={{ width: "100%" }}>
      ...
      `}} />
      <div style={{ width: "100%", overflowX: "auto" }} aria-label="Price over time: past sales, our value, asking and expected sell price">
        <svg className={uid} viewBox={`0 0 ${L.VB_W} ${VB_H}`} preserveAspectRatio="xMidYMid meet" style={{ display: "block", width: "100%", maxWidth: 760, minWidth: 560, height: "auto", margin: "0 auto" }} role="img">
          <defs>
          ...
          {/* gridlines + y labels — ticks sit at 1/4, 2/4, 3/4 of the plot height */}
          {L.ticks.map((tk, i) => {
            const y = L.yBot - ((i + 1) / 4) * (L.yBot - L.yTop);
            return (
              <g key={`g${i}`}>
                <line x1={L.x0} y1={y} x2={L.xNow + 40} y2={y} stroke={C.line} strokeWidth="1" />
                <text x={L.x0 - 8} y={y + 3.5} fontFamily="'IBM Plex Mono', ui-monospace, monospace" fontSize="10" fill={C.faint} textAnchor="end">{fmtMoneyShort(tk)}</text>
              </g>
            );
          })}
          ...
          {/* area + journey (only when we have recorded sales) */}
          {L.hasSales && <path className="an-area" d={L.areaD} fill={`url(#${uid}area)`} />}
          ...
          {!L.hasSales && (
            <text x={(L.x0 + L.xNow) / 2} y={L.yBot - 8} fontFamily="'IBM Plex Mono', ui-monospace, monospace" fontSize="10" fill={C.faint} textAnchor="middle">No recorded sales — showing current estimates</text>
          )}
