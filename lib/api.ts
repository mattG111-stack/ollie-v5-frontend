...
export interface ForSaleRow {
  ...
  buy_price: number | null;        // our estimate of what you could pay
  ...
  /** The site CAN be split — zone, title and size allow it. Unlike
   *  is_subdividable this needs no asking price, so it is the flag the lot
   *  count and the subdivision filter go by. Null on rows loaded before the
   *  column existed. */
  ...
  /** A portal's own published figure — context only, never an input.
   *
   *  The keys used to carry the supplier's name (tm_valuation, oneroof_url),
   *  which put a list of who we buy from on every row of every response,
   *  readable in the network tab. The number still arrives; the key no longer
   *  says where it came from. Which portal is which stays server-side. */
  portal_a_valuation: number | null;
  portal_a_valuation_low: number | null;
  portal_a_valuation_high: number | null;
  /** Already a redirect through our own domain — never the portal's address. */
  portal_a_url: string | null;
  portal_b_valuation: number | null;
  portal_b_valuation_low: number | null;
  portal_b_valuation_high: number | null;
  portal_b_valuation_date: string | null;
...
export interface ImportBatch {
  ...
  note?: string | null;
  is_active: boolean;
  /** staged | preview | published | archived. Needed because "not live" covers
   *  two opposite situations — not published YET, and replaced by a later
   *  load — and is_active alone cannot tell them apart. */
  status?: string;
...
export interface StagedGridRow {
  /** How the vendor is selling it, in the feed's own words, and what we
   *  concluded from that. Shown side by side so a reviewer can see them
   *  disagree — which is what nothing on the review screen could show. */
  sale_method?: string | null;
...
export interface PreferencePreviewRow {
  ...
  max_addl_lots: number | null;
  is_subdividable: boolean | null;
  /** Whether the site can be split at all — what the lot count beside it
   *  means. Needs no asking price, unlike is_subdividable. */
  can_subdivide: boolean | null;
}
