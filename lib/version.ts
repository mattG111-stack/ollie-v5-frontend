 ...
 *
 * That question has been unanswerable so far. A backend can sit twelve days
 * behind its frontend and nothing on the screen says so — every symptom then
 * looks like a new bug rather than an old one that was already fixed but never
 * deployed.
 *
 ...
 * ended at 9.9999998, eight digits deep, and a number nobody can read at a
 * glance cannot do the job described above. 9.99 is the last of it.
 *
 ...
 *     1.12    then
 *
 * Going from 9.99 to 1.0 is not a downgrade. The 9.x numbers were never
