...
import { clearRef, currentRef } from "@/lib/referral";
...
export default function SignUpPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    // Whoever sent them here, if anyone. The server ignores a code it does not
    // recognise, so a stale or wrong one cannot stop the account being created.
    const ref = currentRef();
    try {
      const r = await api<{ token: { access_token: string } }>("/api/auth/sign-up", {
        method: "POST",
        body: JSON.stringify(ref ? { ...data, ref } : data),
      });
