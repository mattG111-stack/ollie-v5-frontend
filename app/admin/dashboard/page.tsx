...
type Metrics = {
  ...
  // Listings whose advertisement has come off the portal. Hidden from every
  // customer list so nobody clicks through to a page that is not there — kept,
  // not deleted, and restored the moment the link answers again.
  off_market_now?: number;
  off_market_7d?: number;
  // Without this a zero above is ambiguous: nothing dropped off, or the check
  // has not run in a fortnight. Both look identical, and only one is fine.
  links_last_checked?: string | null;
  links_never_checked?: number;
};
...
function Inner() {
  ...
  const [who, setWho] = useState<Interests | null>(null);
  ...
  useEffect(() => {
    ...
    api<Interests>("/api/admin/interests?days=90").then(setWho).catch(() => null);
  }, []);
  ...
  return (
    <div className="px-4 sm:px-7 py-6">
      ...
      {/* ENGAGEMENT */}
      <Section title="What each customer is looking for">
        {(who?.rows.length ?? 0) === 0 ? (
          <div className="bg-white border border-line rounded-card shadow-soft p-5 text-sm text-muted">
            Nothing to show yet. This fills in from what people save to a wish
            list, what they ask Ollie, and where they spend their time.
          </div>
        ) : (
          <div className="bg-white border border-line rounded-card shadow-soft overflow-x-auto">
            <table className="w-full text-sm min-w-[860px]">
              <thead className="bg-paper text-[10.5px] uppercase tracking-wider text-faint font-semibold">
                <tr>
                  ...
                  <th className="text-left px-5 py-2.5">Suburbs</th>
                  <th className="text-left px-5 py-2.5">Budget</th>
                  <th className="text-left px-5 py-2.5">Looking for</th>
                  <th className="text-left px-5 py-2.5">Spends time on</th>
                  <th className="text-right px-5 py-2.5">Time</th>
                </tr>
              </thead>
              <tbody>
                {who!.rows.map((r) => (
                  <tr key={r.user_id ?? r.email} className="border-t border-line2 align-top">
                    <td className="px-5 py-2.5">
                      <div className="font-medium">{r.name ?? r.email}</div>
                      {r.name && <div className="text-[11px] text-faint">{r.email}</div>}
                      {r.last_seen && (
                        <div className="text-[11px] text-faint">
                          last seen {new Date(r.last_seen).toLocaleDateString("en-NZ")}
                        </div>
                      )}
                    </td>
                    <td className="px-5 py-2.5">
                      {r.suburbs.length ? r.suburbs.join(", ") : <span className="text-faint">—</span>}
                    </td>
                    <td className="px-5 py-2.5 whitespace-nowrap">
                      {band(r.price_low, r.price_high)}
                      {r.min_beds != null && (
                        <div className="text-[11px] text-faint">{r.min_beds}+ beds</div>
                      )}
                    </td>
                    <td className="px-5 py-2.5">
                      {r.wants.length
                        ? r.wants.join(", ")
                        : <span className="text-faint">no filter saved</span>}
                      {r.last_questions.length > 0 && (
                    ...
                    <td className="px-5 py-2.5 text-muted">
                      {r.top_pages.length ? r.top_pages.join(", ") : "—"}
                      {r.questions > 0 && (
                        <div className="text-[11px] text-faint">
                          {r.questions} question{r.questions === 1 ? "" : "s"} asked
                        </div>
                      )}
                    </td>
        ...
        <p className="text-[11px] text-faint mt-2">
          Built from wish lists, questions asked, and time on each page — over the
          last 90 days. Which individual listings someone opened is deliberately
          not recorded.
        </p>
      </Section>
      ...
      {/* DATA PIPELINE */}
      {/* Above the pipeline section because it is the question the pipeline
          exists to answer: is the number we publish closer to what houses
          actually sell for than the alternatives. */}
      <AccuracyVsHougarden />
      ...
      <Section title="Data pipeline">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="bg-white border border-line rounded-card shadow-soft p-5">
            <div className="text-[11px] uppercase tracking-wider text-faint font-semibold">Come off the market</div>
            <div className="font-display text-3xl font-bold mt-1 tnum">{(m.off_market_now ?? 0).toLocaleString()}</div>
            <div className="text-xs text-muted mt-1">
              {(m.off_market_7d ?? 0).toLocaleString()} in the last 7 days · hidden
              from customers, not deleted
            </div>
            {/* The number above means nothing on its own. A zero reads the same
                whether nothing dropped off or the checker stopped running a
                fortnight ago, so the check's own health sits beside it. */}
            <div className="text-xs mt-2" style={{
              color: m.links_last_checked ? undefined : "#B54708",
            }}>
              {m.links_last_checked
                ? `Links last checked: ${when(m.links_last_checked)}`
                : "Links have never been checked — the daily job has not run yet."}
              {(m.links_never_checked ?? 0) > 0 && m.links_last_checked
                ? ` · ${(m.links_never_checked ?? 0).toLocaleString()} still to check`
                : ""}
            </div>
          </div>
        </div>
...
interface Interests { days: number; rows: Interest[] }

/** A price band as a phrase, because "—" for an open end reads as missing data
 *  when it means "no limit set". */
function band(lo: number | null, hi: number | null): string {
  if (lo == null && hi == null) return "—";
  if (lo != null && hi != null) return `${fmtMoneyShort(lo)}–${fmtMoneyShort(hi)}`;
  if (hi != null) return `up to ${fmtMoneyShort(hi)}`;
  return `${fmtMoneyShort(lo!)}+`;
}
