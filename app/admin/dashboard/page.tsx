...
function Inner() {
  ...
  useEffect(() => {
    api<Metrics>("/api/admin/metrics").then(setM).catch((e) => setErr(e?.detail || e?.message || "Failed to load"));
  ...
  if (!m) return <div className="px-4 sm:px-7 py-6 text-muted text-sm">Loading…</div>;
  ...
  return (
    <div className="px-4 sm:px-7 py-6">
      ...
      <Section title="Data pipeline">
        ...
        <a href="/admin/stages" className="inline-block text-xs text-blue mt-3">Load this week&apos;s data →</a>
      </Section>
    </div>
  );
}
