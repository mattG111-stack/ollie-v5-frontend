...
type Staged = {
  ...
  /** How many listings in this batch a vendor actually named a price for, and
   *  how many are auction / tender / by negotiation. The pair that would have
   *  caught the auction fault before it went live: one export showed 1,436 of
   *  1,586 "asking" a price when the true figure was 308, and nothing on this
   *  screen said so. */
  priced_rows?: number;
  ...
  /** What is on the site RIGHT NOW — independent of whatever is staged.
   *  The week the book fell from 9,281 to 657 there was nothing on any screen
   *  that said so. These are that missing number. */
  live_total?: number;
  live_visible?: number;
  live_gone?: number;
  live_held?: number;
  live_other_hidden?: number;
  /** Week by week, how many houses the link check took off. One number says
   *  the site is smaller; it does not say whether that is normal churn or
   *  something breaking, and those need opposite responses. */
  gone_weekly?: { week_starting: string; taken_off: number }[];
};
...
function Inner() {
  ...
  // Restart says what it did on its own line, because stageMsg is rendered
  // inside a hidden block — a button whose only feedback goes somewhere
  // invisible reads as a button that does nothing.
  ...
  // Listings an earlier short upload archived that are still advertised. Nearly
  // always zero; above zero it is the single most important thing on this page,
  // because it means the site is showing less than it has.
  ...
  const load = useCallback(async () => {
    ...
    // Checked on every load, in both states of this page: after a publish there
    // is nothing staged, and that is exactly when a lost book shows up.
    const r = await api<{ restorable: number }>("/api/admin/release/restorable").catch(() => null);
    setRestorable(r?.restorable ?? 0);
  }, []);
  ...
  // the refusal is load-bearing most of the time (a second heavy worker on one
  // batch is how the container gets OOM-killed), and the operator is the one
  // who can tell "it is working, leave it" from "it has been sitting there for
  // an hour". This is how they say the second one.
  async function restartStage(stage: "enrich" | "price") {
    if (starting) return;
    setStarting("restart");
    setRestartMsg(`Restarting ${stage}…`);
    try {
      const r = await api<StageStarted & { stopped: number }>(
        `/api/admin/release/${stage}/restart`, { method: "POST" });
      setRestartMsg(
        r.stopped > 0
          ? `Stopped the run that was holding this batch and started ${stage} again \u2014 it picks up where the last one got to, so nothing already looked up is repeated.`
          : `Nothing was running, so this just started ${stage}.`);
      if (pollRef.current) clearInterval(pollRef.current);
      const poll = async () => {
        const job = await api<IngestJob>(`/api/admin/jobs/${r.job_id}`).catch(() => null);
        if (!job) return;
        setStageJob(job);
        if (job.status === "completed" || job.status === "failed") {
          if (pollRef.current) clearInterval(pollRef.current);
      ...
      setRestartMsg(`Restart couldn't start: ${e?.detail || e?.message || "request failed"}`);
    } finally {
      setStarting(null);
    }
  }

  // Ask the data provider about one property and say what came back.
  //
  // The three ways it fails need three different responses — refused (wait,
  // it clears), never arrived (network, and nothing is wrong with the
  // addresses), no record (the connection is fine). Told apart they are three
  // different afternoons; told together they are "it didn't work".
  async function testLookup() {
    if (starting) return;
    setStarting("lookup");
    setRestartMsg("Asking the data provider about one property…");
    try {
      const r = await api<{ ok: boolean; status: string; headline: string; detail: string; address: string }>(
        "/api/admin/release/lookup-check", { method: "GET" });
      setRestartMsg(`${r.ok ? "✓" : "✗"} ${r.headline} ${r.detail}`);
    } catch (e: any) {
      setRestartMsg(`Couldn't run the check: ${e?.detail || e?.message || "request failed"}`);
    } finally {
      setStarting(null);
    }
  }
  ...
  // Put back listings an earlier short upload archived. Nothing is ever deleted
  // — archiving a batch only hides it — so this needs no file and no re-upload.
  async function restoreBook() {
    if (!confirm(
      `Put ${restorable.toLocaleString()} listing(s) back on the site?\n\n` +
      `These were archived by an upload smaller than the book, and their ` +
      `advertisements are still up. They come back exactly as they were.`
    )) return;
    setBusy(true); setRestoreMsg(null);
    try {
      const r = await api<{ restored: number }>("/api/admin/release/restore", { method: "POST" });
      setRestoreMsg(`${r.restored.toLocaleString()} listing(s) are back on the site.`);
      await load();
    } catch (e: any) {
      setRestoreMsg(e?.detail || e?.message || "Could not restore");
    } finally {
      setBusy(false);
    }
  }

  // Deliberately loud, and shown in BOTH states of this page. The fault it
  // reports is invisible everywhere else: the site simply has fewer houses on
  // it than it did, and no screen says so.
  const RestoreBanner = () =>
    restorable > 0 || restoreMsg ? (
      <div className="bg-white border border-danger rounded-card shadow-soft p-4 mt-4">
        {restorable > 0 && (
          <>
            <div className="font-display font-semibold">
              {restorable.toLocaleString()} listing(s) are missing from the site
            </div>
            <p className="text-sm text-muted mt-1">
              An earlier upload was smaller than the book and archived the rest.
              Nothing was deleted — these listings are still here, and their
              advertisements are still up. Putting them back needs no re-upload.
            </p>
            <button
              onClick={restoreBook}
              disabled={busy}
              className="mt-3 px-3 py-1.5 text-xs font-semibold text-white rounded-md disabled:opacity-50"
              style={{ background: "#0A8754" }}
            >
              Put {restorable.toLocaleString()} listing(s) back
            </button>
          </>
        )}
        {restoreMsg && <div className="text-sm mt-2" style={{ color: "#0A8754" }}>{restoreMsg}</div>}
      </div>
    ) : null;


  /* What a customer can see, and what has been taken off. Shown in BOTH states
     of this page: after a publish there is nothing staged, and that is exactly
     when somebody wants to know how many listings are live. */
  const LiveNow = () =>
    s && (s.live_total ?? 0) > 0 ? (
      <div className="bg-white border border-line rounded-card shadow-soft p-4 mt-4">
        <div className="flex items-baseline justify-between">
          <div className="font-display font-semibold">On the site now</div>
          <CsvButton />
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-3">
          <Stat label="Live listings" value={(s.live_visible ?? 0).toLocaleString()} />
          <Stat label="Off — advertisement gone"
                value={(s.live_gone ?? 0).toLocaleString()}
                sub="found sold or withdrawn by the daily link check" />
          <Stat label="Held at review" value={(s.live_held ?? 0).toLocaleString()}
                accent={(s.live_held ?? 0) > 0} />
          <Stat label="Hidden — other" value={(s.live_other_hidden ?? 0).toLocaleString()}
                sub="no floor area, or a price the scraper invented" />
        </div>
        <div className="text-[11px] text-faint mt-2">
          {(s.live_total ?? 0).toLocaleString()} listings in the live load in total.
        </div>
        {(s.gone_weekly?.length ?? 0) > 0 && (
          <div className="mt-4 pt-3 border-t border-line">
            <div className="text-[10px] uppercase tracking-wider text-faint font-semibold">
              Taken off, week by week
            </div>
            <div className="flex gap-4 mt-2 flex-wrap">
              {s.gone_weekly!.map((w) => (
                <div key={w.week_starting} className="min-w-[86px]">
                  <div className="font-display text-lg font-bold tnum">
                    {w.taken_off.toLocaleString()}
                  </div>
                  {/* Day, month, year — never the ISO string. */}
                  <div className="text-[10.5px] text-muted">
                    w/c {new Date(w.week_starting + "T00:00:00").toLocaleDateString("en-NZ", {
            ...
            <div className="text-[10.5px] text-faint mt-2">
              Houses whose advertisement the daily check opened and found gone.
              Counted once each, however many loads they appeared in.
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div className="px-4 sm:px-7 py-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mt-4">
        ...
        {/* THE TILE THAT WAS MISSING. Four Auckland houses in five sell by
            auction, tender or negotiation and name no price — so a batch where
            nine in ten "advertise" one is a batch reading a search price as an
            asking price, which is exactly what happened and went live. Anybody
            who knows this market stops at 91%; nobody was shown the number.
            Flagged above 60% rather than left as a neutral count, because a
            figure only wrong to someone already suspicious is not a check. */}
        <Stat
          label="Vendor named a price"
          value={`${(s.priced_rows ?? 0).toLocaleString()}/${(s.forsale_rows ?? 0).toLocaleString()}`}
          accent={
            (s.forsale_rows ?? 0) > 50 &&
            (s.priced_rows ?? 0) / (s.forsale_rows || 1) > 0.6
          }
          sub={
            s.forsale_rows
              ? `${(s.unpriced_rows ?? 0).toLocaleString()} sell by auction, tender or negotiation` +
                ((s.priced_rows ?? 0) / s.forsale_rows > 0.6
                  ? " — high for this market, check how it sells in the grid"
                  : "")
              : undefined
          }
        />
      ...
      <div className="bg-white border border-line rounded-card shadow-soft p-4 mt-5">
        ...
        <div className="flex flex-wrap items-center gap-3 mt-3">
          ...
          {/* The way out of a run that is neither finished nor going anywhere.
              The button above refuses while one is in flight — two enrichers on
              one batch is what kills the container — so without this the only
              cure for a stuck run was to wait the lock out. It takes the stage
              off whoever is holding it and starts again; because enrich resumes,
              nothing already looked up is paid for twice. */}
          <StageButton label="Restart enrich"
            running={starting === "restart"}
            onClick={() => restartStage("enrich")} />
          {/* "The lookup isn't working" had no way to be asked. The only
              instrument was to run an enrich over a whole batch and read the
              summary afterwards — a twenty-minute question with a one-second
              answer in it. One real lookup, on demand. */}
          <StageButton label="Test the lookup"
            running={starting === "lookup"}
            onClick={testLookup} />
...
/* The live data as a CSV — same rows, same columns as the workbook, in the
 * format a spreadsheet opens without asking anything. Sits next to the live
 * counts because that is where somebody looks at a number and wants the rows
 * behind it. */
function CsvButton() {
  const [busy, setBusy] = useState(false);
  ...
  async function download() {
    ...
    try {
      ...
      const res = await fetch("/api/properties/export.csv", {
      ...
      a.href = url;
      const d = new Date();
      a.download = `apex_live_data_${String(d.getDate()).padStart(2, "0")}-${String(d.getMonth() + 1).padStart(2, "0")}-${d.getFullYear()}.csv`;
  ...
  return (
    <button onClick={download} disabled={busy}
      title="Every listing in the live load, including the ones held back or taken off, with a column saying which is which"
      className="text-[11px] font-semibold text-blue hover:underline disabled:opacity-50">
      {busy ? "Preparing…" : "⤓ Download live data (CSV)"}
    </button>
  );
}
