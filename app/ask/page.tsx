...
function Inner() {
  ...
  return (
    ...
    >
      {progress && <Counting pct={progress.pct} phase={progress.phase} />}
        ...
        <>
          ...
          <div
            style={{
              ...
              marginTop: msgs.length ? 10 : "min(6vh, 44px)",
              // Idle fills only the middle of its canvas, so the empty lower
              // half opens a gap between him and the question box. Pull the
              // rest of the page up through it — but ONLY once there is a
              // conversation. On the empty page the title is inside this block
              // and already pulled up into the canvas itself, so a second pull
              // here drags the box over the top of it.
              marginBottom: msgs.length ? -Math.round(orbSize * 0.17) : 0,
              ...
              <>
                <h1
                  style={{
                    ...
                    margin: `${-Math.round(orbSize * 0.22)}px 0 0`,
                  }}
                >
                  {t("ask.title")}
                </h1>
                ...
                {busy && (
                  <p
                    style={{
                      fontSize: 15, color: D.dim, lineHeight: 1.5,
          ...
          {quota?.shared && quota.configured && quota.limit != null && (
            <div style={{ marginTop: 12, fontSize: 13, color: quota.remaining === 0 ? "#FF9A6E" : D.faint }}>
              {quota.remaining === 0
          ...
          {msgs.length > 0 && (
            <div style={{ width: "100%", maxWidth: 760, marginTop: 22,
          ...
          {prefs && prefs.state !== "unset" && (
            <button
              ...
              style={{
                ...
                fontFamily: "inherit", fontSize: 12.5, color: D.faint, textAlign: "center",
 ...
 */
function Counting({ pct, phase }: { pct: number; phase: string | null }) {
  const R = 13;
