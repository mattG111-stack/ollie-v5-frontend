...
function MarketDirection({ s }: { s: SuburbStats | null }) {
  ...
  // The months behind us are history and are drawn in ink. Only the LAST step —
  // the one into the month we are in — carries a colour, so red and green on
  // this line mean "now", not "since the start of the chart". The run into a
  // part-counted month is dashed as well, so an incomplete figure never reads
  // as a finished one.
  const hasPartial = partialIdx >= 0 && known.some(([i]) => i === partialIdx);
  const solid = pathOf(known);
  const lead = known.length > 1 ? pathOf(known.slice(-2)) : "";
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} className="mt-1 overflow-visible"
        ...
        {lead && (
          <path d={lead} fill="none" stroke={dirColour} strokeWidth="2"
                strokeDasharray={hasPartial ? "4 4" : undefined}
