...
type SuburbOption = {
  suburb: string;
  sold: number;   // sales in the live sold batch
  live: number;   // listings in the live for-sale batch
};
...
function Inner() {
  ...
  // The suburbs this region's live batches actually cover.
  useEffect(() => {
    let alive = true;
    api<SuburbOption[]>("/api/properties/suburbs")
      .then((r) => {
        if (!alive) return;
        setOptions(r);
        // Land on a suburb that exists. Defaulting to a hard-coded "Remuera"
        // that is not in the batch opens the page on an empty panel and reads
        // as the app being broken.
        if (r.length && !r.some((o) => o.suburb === "Remuera")) {
          setSuburb([...r].sort((a, b) => b.sold - a.sold)[0].suburb);
        }
      })
    ...
    <div className="px-7 py-6">
      ...
      <div className="bg-white border border-line rounded-card shadow-soft p-5 mb-5">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <SuburbSelect value={suburb} onPick={setSuburb} options={options} loading={optsLoading} />
 ...
 * The suburb picker.
 *
 * This was a free-text box backed by type-ahead. A typo, a spelling the feed
 * does not use ("Mt Eden" for "Mount Eden"), or a real suburb that simply is not
 * in this region's batch all produced the same thing — an empty panel with no
 * explanation, and no way to tell which of the three had happened. The list is
 * now built from the data, so every option has something behind it and nothing
 * can be typed wrong.
 *
 ...
 */
function SuburbSelect({ value, onPick, options, loading }: {
  value: string;
  onPick: (s: string) => void;
  options: SuburbOption[];
  loading: boolean;
}) {
  // The selected suburb might not be in the list — a stale link, or a region
  // whose batch has moved on. Keep it selectable rather than silently snapping
  // the picker to something the user did not choose.
  const known = options.some((o) => o.suburb === value);
  ...
  return (
    <select
      ...
      className="bg-paper border border-line rounded-lg px-3 py-2 text-sm w-64 focus:outline-none focus:border-blue disabled:opacity-60"
      aria-label="Suburb"
    >
      {!known && value && <option value={value}>{value}</option>}
      {loading && <option value={value}>Loading suburbs…</option>}
      {!loading && options.length === 0 && <option value={value}>No suburbs in this batch</option>}
      {options.map((o) => (
...
function MarketDirection({ s }: { s: SuburbStats | null }) {
  const [metric, setMetric] = useState<(typeof SERIES)[number]["key"]>("median_price");
