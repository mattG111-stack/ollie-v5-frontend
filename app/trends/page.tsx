...
function MarketDirection({ s }: { s: SuburbStats | null }) {
  ...
  // The big number is the newest COMPLETED month, not the month we are in.
  // Remuera showed "38 days" from two August sales beside a tile reading 45 for
  // the year — two different measurements wearing the same label. The part
  // month stays on the chart, dashed; it just does not get to be the headline.
  const headline = settled.length ? settled[settled.length - 1] : last;
  const headlineMonth = pts[headline[0]]?.month;
  ...
  return (
    <div className="mt-4 pt-4 border-t border-line/60">
      ...
      <div className="flex items-baseline gap-2">
        <div className="font-display text-2xl font-bold tabular-nums">{fmtV(headline[1])}</div>
        {headlineMonth && (
          <div className="text-[11px] text-faint">in {monthShort(headlineMonth)}</div>
        )}
        ...
        <div className={`text-xs font-semibold ${
          ...
          {flat ? "holding steady"
            : `${delta > 0 ? "▲" : "▼"} ${improving ? "improving" : "softening"} vs the 3 months before`}
        </div>
      </div>
