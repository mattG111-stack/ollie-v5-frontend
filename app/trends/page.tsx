...
import { SERIES as MD_SERIES, marketVerdict } from "@/lib/marketDirection";
import type { MetricKey } from "@/lib/marketDirection";
...
type SuburbStats = {
  ...
  monthly: MonthPoint[];
  by_beds: BedSeries[];
  age_selling_times?: { groups: { key: string; sales: number; median_days: number | null }[]; excluded_sales: number } | null;
...
/** Where the page opens, in order of preference. Mount Wellington by choice.
 *
 *  An ordered list rather than one name so that a suburb missing from a given
 *  week lands on the next recognisable one instead of on whatever happens to
 *  have the most sales — a default that moves with the data opens the page
 *  somewhere different each week for no reason a reader can see.
 */
const PREFERRED_SUBURBS = [
  "Mount Wellington", "Remuera", "Auckland Central", "Mount Eden", "Takapuna",
];
...
function Inner() {
  const { t } = useT();
  const [suburb, setSuburb] = useState(PREFERRED_SUBURBS[0]);
  // One kind of home at a time. A suburb median across houses AND apartments
  // moves on the mix of what sold rather than on the market, and that swamps
  // the trend it is meant to show. "" = every type, which is where it starts so
  // the page opens on the same figures it always has.
  const [ptype, setPtype] = useState("");
  const [data, setData] = useState<SuburbTrend | null>(null);
  const [loading, setLoading] = useState(false);
  const { options, loading: optsLoading } = useSuburbs();

  // Land on the preferred suburb, MATCHED LOOSELY AND ADOPTED EXACTLY.
  //
  // The old check compared a hard-coded name to the suburb string in the data
  // with ===, so a difference of case or a trailing space read as "not in the
  // batch" for a suburb sitting right there in the dropdown, and the page fell
  // through to whichever suburb had the most sales. Comparing trimmed and
  // lower-cased fixes that; the string that gets selected is the data's own,
  // because that is what the trend query is keyed on.
  //
  // `touched` keeps all of this out of the way the moment a person chooses for
  // themselves — otherwise a late-arriving options list overwrites their pick.
  const [touched, setTouched] = useState(false);
  useEffect(() => {
    if (touched || !options.length) return;
    const norm = (v: string) => v.trim().toLowerCase();
    if (options.some((o) => norm(o.suburb) === norm(suburb))) return;   // already fine
    const present = PREFERRED_SUBURBS
      .map((s) => options.find((o) => norm(o.suburb) === norm(s)))
      .find(Boolean);
    setSuburb(present ? present.suburb
                      : [...options].sort((a, b) => b.sold - a.sold)[0].suburb);
  }, [options, suburb, touched]);

  const chooseSuburb = (s: string) => { setTouched(true); setSuburb(s); };
    ...
    <div className="px-7 py-6">
      ...
      <div className="bg-white border border-line rounded-card shadow-soft p-5 mb-5">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <SuburbSelect value={suburb} onChange={chooseSuburb} allLabel={null} width={256} />
          <select
            value={ptype}
            onChange={(e) => setPtype(e.target.value)}
            className="apex-suburb-select bg-white border border-line rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue"
            aria-label="Property type"
          >
            <option value="">{t("trends.allTypes")}</option>
            <option value="House">{t("ptable.house")}</option>
            <option value="Townhouse">{t("ptable.townhouse")}</option>
            <option value="Apartment">{t("ptable.apartment")}</option>
            <option value="Unit">{t("ptable.unit")}</option>
          </select>
          <span className="text-xs text-muted">{t("trends.orPick")}</span>
          {(quick.length ? quick.map((o) => o.suburb) : POPULAR_SUBURBS).map((s) => (
            <button
              key={s}
              onClick={() => chooseSuburb(s)}
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                    <div>
                      ...
                      <Sparkline values={data.points.map((p) => p.median_asking)} width={400} height={70} color="#454B54" />
                    </div>
                    <div>
...
function SuburbStatsPanel({ suburb, ptype, trend }:
  { suburb: string; ptype?: string; trend?: number[] }) {
  ...
  return (
    <div className="bg-white border border-line rounded-card shadow-soft p-5 flex flex-col">
      ...
      {/* Three from 640px, two on a normal phone, ONE on a small one.

          The middle step was already here: at 390px three tiles are 110px each
          and "Median days to sell" wraps onto three lines, which makes a short
          label look like a paragraph. Two across fixed that and was assumed to
          hold all the way down — it does not. At 360px, which is a very common
          phone, two across is 126px a tile and the label wraps until the tile
          is 125px TALL: a square of stacked words with a number somewhere in
          it. Measured, not guessed: it is what the responsive suite reports the
          moment that suite is able to run at all. */}
      <div className="grid grid-cols-1 min-[400px]:grid-cols-2 sm:grid-cols-3 gap-2.5 mt-4">
        <StatTile
          k="Sales this month"
      ...
      <MoveRow icon="＋🛁" name="Add a bathroom" note={basisNote(bath)}
        ...
        last />
      {/* This described a method that is no longer used — a regression holding
          floor area constant, which answered "what if this house were divided
          into more rooms" and once reported minus $210,000 a bedroom. What the
          panel does now is compare what 3-bedroom homes sell for here against
          4-bedroom ones, so the text has to say that. */}
      <div className="text-[11px] text-faint mt-3">
        What homes with one more bedroom actually sell for here, against those
        with one fewer — the same kind of home, so it is a like-for-like
        comparison rather than a house against an apartment. Where the sales are
        too varied to separate the room from the house around it, this says so
      ...
      {trend && trend.filter((v) => v != null && v > 0).length >= 2 && (
        <div className="mt-4 pt-4 border-t border-line/60">
          ...
          <Sparkline values={trend} width={440} height={64} color="#454B54" />
        </div>
      )}
    </div>
  );
...
// cancels out.
// SERIES and the verdict rule live in lib/marketDirection.ts. They were here,
// they have been fixed twice, and the fix came back both times because nothing
// in React is testable without a browser. Now e2e/marketDirection.spec.ts pins
// every direction for every metric.
const SERIES = MD_SERIES;
...
function MarketDirection({ s }: { s: SuburbStats | null }) {
  const [metric, setMetric] = useState<MetricKey>("sale_vs_cv");
  ...
  // By CALENDAR month, with the part-counted month nulled out — the verdict
  // steps back three months, so it needs the gaps, not a compacted list.
  const V = marketVerdict(
    raw.map((v, i) => (i === partialIdx ? null : (v ?? null))), metric);
  const { delta, improving, flat } = V;
  ...
  // Ink for history; colour for the window the verdict is actually about.
  //
  // The verdict above compares the last three completed months with the three
  // before them — deliberately, because one quiet month is not a trend. So the
  // COLOURED part of the line has to be those three months, not just the final
  // step. Painting only the last step meant a line could fall while wearing the
  // colour of a six-month comparison that went the other way: on "days to sell"
  // that drew a DOWNWARD step in RED, which reads as "selling faster is bad"
  // when it is the opposite. Now what is coloured is what the sentence above it
  // is describing.
  //
  ...
  // chosen metric has a value in any of them. Pick a property type whose sales
  // carry no days-on-market, or whose months were each too thin to publish a
  // median, and `known` is empty while `monthly` is not: this line then read
  // element [0] of an empty array and threw, white-screening the whole trends
  // page. Reported as "select house, townhouse, apartment, unit and it
  // crashes", which is exactly when it happens.
  const verdictFrom = settled.length >= 3
    ? settled[settled.length - 3][0]
    : (settled[0]?.[0] ?? known[0]?.[0] ?? 0);
  const startIdx = Math.max(0, known.findIndex(([i]) => i === verdictFrom));
  const solid = pathOf(known.slice(0, startIdx + 1));
  const lead = pathOf(hasPartial ? known.slice(startIdx, -1) : known.slice(startIdx));
  // The run into a part-counted month stays dashed, so an incomplete figure
  // never reads as a finished one.
  const dashed = hasPartial && known.length > 1 ? pathOf(known.slice(-2)) : "";
  const last = known[known.length - 1] ?? [0, 0];
  // The big number is the window the VERDICT is about — the last three
  // completed months — not the single newest one.
  //
  // It used to be the newest completed month, and that is how a suburb running
  // 70 days in January, ~20 in April and May, then 44 in June rendered as
  // "44 days in Jun 26 ▼ improving vs the 3 months before". Both halves true:
  // 44 is June, and Apr-Jun (28.7) really is an improvement on Jan-Mar (57.7).
  // Side by side they say "44 days is an improvement", which is not what either
  // half means. A number and a verdict on one line have to be about one thing.
  //
  // The part-counted month stays on the chart, dashed, and out of both.
  const headlineValue = V.value;
  const headlineMonth = V.index >= 0 ? pts[V.index]?.month : undefined;
    ...
    <div className="mt-4 pt-4 border-t border-line/60">
      ...
      <div className="flex items-baseline gap-2">
        <div className="font-display text-2xl font-bold tabular-nums">
          {headlineValue == null ? "—" : fmtV(headlineValue)}
        </div>
        {headlineMonth && (
          <div className="text-[11px] text-faint">
            3 months to {monthShort(headlineMonth)}
          </div>
        )}
        {/* The arrow points where the NUMBER went; the colour and the word say
            whether that is good. For every metric but one those are the same
            thing, which is how the arrow came to be drawn from `improving`.
            Days to sell is the exception: fewer days is a better market, so a
            falling figure was being shown as "▼ softening" in red — a down
            arrow next to the word for getting worse, on a number that had gone
            down. It read as "days fell and that is bad", which is the opposite
            of what it means. */}
        <div className={`text-xs font-semibold ${
          flat ? "text-faint" : improving ? "text-under" : "text-danger"}`}>
          {V.thin ? ""
            : flat ? V.word
            : `${V.arrow} ${V.word} vs the ${V.monthsBack} months before`}
        </div>
      </div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} className="mt-1 overflow-visible"
        ...
        <path d={solid} fill="none" stroke="#454B54"
        ...
        {dashed && (
          <path d={dashed} fill="none" stroke={dirColour} strokeWidth="2"
                strokeDasharray="4 4" strokeLinejoin="round" strokeLinecap="round" />
        )}
        <circle cx={x(last[0])} cy={y(last[1])} r="3.5"
                fill={hasPartial ? "#fff" : dirColour}
                stroke={hasPartial ? dirColour : "none"} strokeWidth="2" />
      </svg>
...
/** Which sales a window figure was taken over — "in 2026", "2019–2026".
 *
 *  The tile and the monthly chart both say "days to sell" and they are not the
 *  same measurement: the tile is every sale in the chosen window, the chart is
 *  one month at a time. Unlabelled, a tile reading 45 beside a chart reading 38
 *  looks like one of them is broken. */
function windowLabel(s: SuburbStats | null): string | undefined {
  if (!s?.from_year || !s?.to_year) return undefined;
  return s.from_year === s.to_year ? `in ${s.to_year}` : `${s.from_year}–${s.to_year}`;
}
