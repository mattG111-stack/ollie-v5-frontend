"use client";

import Link from "next/link";
import { useSearchParams } from "next/navigation";
import { Suspense, useMemo, useState } from "react";
import AppShell from "@/components/AppShell";
import PropertiesTable from "@/components/PropertiesTable";
import { useAuth } from "@/lib/auth";

// Same-origin only (see lib/api.ts) — never a cross-origin call, so no CORS.
const API_BASE = "";

export default function PropertiesPage() {
  return (
    <AppShell>
      <Inner />
    </AppShell>
  );
}

function Inner() {
  const { me } = useAuth();
  const isAdmin = me?.role === "admin";

  return (
    <div className="px-7 py-6">
      {/* Stacks on a phone. Side by side, the buttons below are
          whitespace-nowrap and will not shrink, so the heading was left with
          whatever width remained — about one word — and wrapped down the page
          while "Export" still ran off the right edge. min-w-0 lets the text
          column shrink at all: a flex child defaults to min-width:auto, which
          means it refuses to go narrower than its longest word. */}
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        <div className="min-w-0">
          <div className="text-[11px] uppercase tracking-widest text-blue font-semibold">All properties</div>
          <h1 className="font-display text-2xl font-semibold mt-1.5">Every listing, one filtered view</h1>
          <p className="text-sm text-muted mt-1">
            Scan the whole market like a spreadsheet — sort any column, filter by opportunity, click a row to open it.
          </p>
        </div>
        <div className="flex items-center gap-2.5 flex-wrap sm:flex-nowrap sm:shrink-0">
          <OpenOnMapLink />
          {isAdmin && <ExportButton />}
          {isAdmin && <AuditButton />}
        </div>
      </div>
      <Suspense fallback={<div className="text-sm text-muted">Loading…</div>}>
        <PropertiesWithSearch />
      </Suspense>
    </div>
  );
}

/** A plain link to the map view, so it can be bookmarked or shared as a link
 *  rather than only reached by toggling once the page has already loaded. */
function OpenOnMapLink() {
  return (
    <Link
      href="/properties?view=map"
      className="border border-line bg-card hover:bg-paper text-text px-4 py-2.5 rounded-lg font-semibold text-sm whitespace-nowrap"
    >
      ◉ Open on map
    </Link>
  );
}

/* The live data as a workbook, with the checks already run.
 *
 * Separate from the CSV rather than replacing it: the CSV is the raw dump for
 * anybody who wants to do their own thing with it, and this is the one you open
 * when you want to know whether the data is right. It leads with a sheet of
 * checks — each a question, the answer beside it, and what the answer should
 * look like — because a spreadsheet of nine thousand rows is not an audit.
 *
 * It also includes rows a customer CANNOT see, which the site by definition
 * never shows and which is exactly where a fault hides. */
function AuditButton() {
  const [busy, setBusy] = useState(false);

  async function download() {
    if (busy) return;
    setBusy(true);
    try {
      const token = localStorage.getItem("ollie_token");
      const res = await fetch(`${API_BASE}/api/properties/export.xlsx`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      if (!res.ok) {
        const text = await res.text().catch(() => "");
        throw new Error(text || `HTTP ${res.status}`);
      }
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      const d = new Date();
      const stamp = `${String(d.getDate()).padStart(2, "0")}-${String(d.getMonth() + 1).padStart(2, "0")}-${d.getFullYear()}`;
      a.download = `apex_live_data_${stamp}.xlsx`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
    } catch (e: any) {
      alert(`Download failed: ${e?.message ?? "unknown"}`);
    } finally {
      setBusy(false);
    }
  }

  return (
    <button
      onClick={download}
      disabled={busy}
      title="Every live listing plus the ones held back, with a sheet of checks in front of it"
      className="bg-blue text-white hover:bg-blue-dark disabled:opacity-50 px-4 py-2.5 rounded-lg font-semibold text-sm whitespace-nowrap"
    >
      {busy ? "Preparing…" : "⤓ Live data + checks (Excel)"}
    </button>
  );
}

function ExportButton() {
  const [busy, setBusy] = useState(false);

  async function download() {
    if (busy) return;
    setBusy(true);
    try {
      const token = localStorage.getItem("ollie_token");
      const res = await fetch(`${API_BASE}/api/properties/export.csv`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      if (!res.ok) {
        const text = await res.text().catch(() => "");
        throw new Error(text || `HTTP ${res.status}`);
      }
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      const stamp = new Date().toISOString().slice(0, 10);
      a.download = `apex_priced_export_${stamp}.csv`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
    } catch (e: any) {
      alert(`Export failed: ${e?.message ?? "unknown"}`);
    } finally {
      setBusy(false);
    }
  }

  return (
    <button
      onClick={download}
      disabled={busy}
      className="bg-blue text-white hover:bg-blue-dark disabled:opacity-50 px-4 py-2.5 rounded-lg font-semibold text-sm whitespace-nowrap"
    >
      {busy ? "Preparing…" : "⤓ Export all to CSV"}
    </button>
  );
}

function PropertiesWithSearch() {
  const params = useSearchParams();
  // Everything the shared filter chain already understands, readable from the
  // URL. Added so "I'll open on your areas" can be true: when Ollie hands
  // somebody off to this page after they've told him what they're hunting, the
  // list they land on is the one he described, not the whole city.
  //
  // Passed straight through as strings — the API validates them, and a value
  // typed into the address bar has to be refused there rather than trusted
  // here. Anything not on this list is ignored.
  const initialFilter = useMemo(() => {
    const filter: Record<string, string> = {};
    for (const key of [
      "search", "suburb", "district", "min_price", "max_price",
      "min_beds", "underpriced", "subdividable", "cashflow_positive",
    ]) {
      const v = params?.get(key);
      if (v) filter[key] = v;
    }
    return filter;
  }, [params]);

  // All properties opens on the MAP. ?view=list overrides it for one visit, and
  // toggling is remembered — but the map is what this page opens on by default,
  // which "opens on the map" has to mean for someone arriving fresh.
  const viewParam = params?.get("view");
  const initialView = viewParam === "map" ? "map" : viewParam === "list" ? "list" : undefined;

  return (
    <PropertiesTable
      initialFilter={initialFilter}
      initialView={initialView}
      defaultView="map"
      viewMemoryKey="apex:properties:view"
    />
  );
}
