      ...
      >
        ...
        <Card>
          ...
          {(() => {
            ...
            // NB: CoreLogic (propertyvalue.co.nz) is used INTERNALLY only — to fill
            // gaps and sanity-check our data — and is deliberately NOT shown as a
            // source here. The external suggestions we surface are Trade Me,
            // realestate.co.nz and homes; CoreLogic's figures never reach the user.
      ...
      {(p.valuation_last_sold_value || parseSaleHistory(p.sale_history_json).length > 0) && (
        <Card style={{ marginTop: 24 }}>
          ...
          {/* Price-over-time: past sales plotted against when they sold, with
              today's asking (and our estimate + CV) on the same timeline. Falls
              back to the single last-sold point when that's all we have. */}
          {(() => {
