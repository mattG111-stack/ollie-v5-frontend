      ...
      >
        ...
        <Card>
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <Row
              label={t("prop.vsCv")}
              value={fmtPct(p.pred_vs_cv)}
              tone={(p.pred_vs_cv ?? 0) >= 0 ? "good" : "bad"}
            />
            <Row
            ...
            />
            <Row
              label={t("prop.marginVsList")}
              value={
                p.fair_value != null && p.asking_price != null && p.margin != null
                  ? `${p.margin > 0 ? "+" : "−"}${fmtMoneyShort(
                      Math.abs(p.fair_value - p.asking_price),
                    )} (${p.margin > 0 ? "+" : ""}${(p.margin * 100).toFixed(1)}%)`
                  : "—"
              }
              strong
              tone={p.margin != null ? (p.margin >= 0.05 ? "good" : p.margin <= -0.05 ? "bad" : undefined) : undefined}
            />
          ...
          {(() => {
            ...
            if (!p.is_premium && p.fair_value != null)
              tiles.push({ label: t("prop.srcOllie"), value: p.fair_value, ollie: true });
            if (hgIndependent)
              tiles.push({
            ...
            // propertyvalue.co.nz (CoreLogic) — their AVM mid, range as the sub.
            ...
            return (
              <div style={{ marginTop: 18, paddingTop: 16, borderTop: `1px solid ${C.divider}` }}>
                ...
                {ext?.pv_discrepancies && ext.pv_discrepancies.length > 0 && (
                  <div style={{
                    marginTop: 12, padding: "10px 12px", borderRadius: 10,
                    background: "rgba(212,80,62,.06)", border: "1px solid rgba(212,80,62,.25)",
                  }}>
                    <div style={{ fontFamily: MONO, fontSize: 9.5, letterSpacing: ".1em", color: C.danger, textTransform: "uppercase", fontWeight: 700 }}>
                      {t("prop.pvCheckTitle")}
                    </div>
                    <div style={{ marginTop: 6, display: "flex", flexDirection: "column", gap: 3 }}>
                      {ext.pv_discrepancies.map((d) => (
                        <div key={d.field} style={{ fontSize: 11.5, color: C.ink }}>
                          <strong>{t(`prop.pvField.${d.field}`)}</strong>: {t("prop.pvOursTheirs", { ours: pvFmt(d.field, d.ours), theirs: pvFmt(d.field, d.theirs) })}
                        </div>
                      ))}
                    </div>
                    <div style={{ fontSize: 10.5, color: C.faint, marginTop: 6 }}>{t("prop.pvCheckNote")}</div>
                  </div>
                )}
                {ext?.pv_gaps && ext.pv_gaps.length > 0 && (
                  <div style={{ marginTop: 10, fontSize: 11.5, color: C.faint }}>
                    {t("prop.pvGapsIntro")}{" "}
                    {ext.pv_gaps.map((g, i) => (
                      <span key={g.field}>
                        {i > 0 ? ", " : ""}
                        <strong style={{ color: C.ink }}>{t(`prop.pvField.${g.field}`)}</strong> {pvFmt(g.field, g.theirs)}
                      </span>
                    ))}
                    .
                  </div>
                )}
                {/* Last sold (CoreLogic) is still captured in the data, just not
                    shown on the property page. Re-enable here if wanted. */}
              </div>
            );
          })()}
        </Card>
