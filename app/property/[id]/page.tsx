...
function Inner({ id }: { id: string }) {
  ...
  // when the council record values the land and not the house, and this page
  // divided by that CV anyway and printed the result ("+206.7%", "+2,296.9%").
  // One number, decided in one place — the page shows it or shows nothing.
  const vsCv = p.pred_vs_cv ?? null;
  ...
  return (
    <div style={{ padding: isMobile ? "20px 16px 48px" : "34px 40px 60px", maxWidth: 1500, width: "100%" }}>
      ...
      {/* The advertisement has come off the portal. This page is still reachable
          ...
          so it says so plainly rather than presenting a sold or withdrawn house
          as though it were on the market and letting somebody ring an agent
          about it. It is not an error: the numbers below are still the last
          thing we knew, which is the reason to keep the page at all. */}
      {p.off_market && (
        <div
          role="status"
          style={{
            ...
            color: "#7A4A00", fontSize: 14, lineHeight: 1.5,
          }}
        >
          <strong>No longer advertised.</strong> This listing has come off the
          market — it may have sold, or been withdrawn. The figures below are the
          last we recorded.
        </div>
      )}
      ...
      <div
        style={{
          display: "grid",
          // One column on a phone. Side by side, the hero card and the photo
          // each got about 170px and every figure in the card was clipped
          // mid-number. Nothing overflowed, so the responsive suite passed it.
          gridTemplateColumns: isMobile ? "1fr" : shots.length ? "1.35fr 1fr" : "1fr",
          gap: 16,
          marginTop: 18,
          alignItems: "stretch",
        }}
          ...
          >
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 18 }}>
                {/* A house going to auction has no asking price. Saying
                    "Asking —" claims we do not know; saying "Auction" is both
                    true and the more useful thing to a buyer. */}
                <HeroChip>
                  {p.asking_price != null
          ...
          >
            ...
            <div style={{ padding: "24px 30px", borderRight: "1px solid rgba(255,255,255,.1)" }}>
              ...
              <div
                className="tnum"
                style={{
                  fontSize: p.can_subdivide && p.max_addl_lots ? 38 : 26,
                  fontWeight: 900,
                  letterSpacing: "-.03em",
                  color: "#C9CED6",
                  marginTop: 8,
              ...
              >
                {/* The number of extra lots the land supports is a fact about
                    the land. Gating it on the profit flag meant a site that
                    reached this page THROUGH the subdivision filter then said
                    "not flagged" on it. */}
                {p.can_subdivide && p.max_addl_lots
      ...
      <div style={{ marginTop: 30 }}>
        ...
        <div
          style={{
            display: "grid",
            // Six fixed columns put each spec in ~60px on a phone, so "447 m²"
            // wrapped onto three lines and the tile grew to 133px tall. 140px is
            // the width at which a floor area stays on one line, which gives two
            // per row on a phone and all six across on a desktop.
      ...
      >
        ...
        <Card>
          ...
          <div style={{ marginTop: 16 }}>
            <Row
              label={t("prop.listPrice")}
              value={
                // A vendor who has stopped naming a price has not stopped
                // having one, and the last figure they DID name is a fact
                // about this listing. State it, with the day it was showing,
                  ...
                  <span style={{ display: "inline-block", textAlign: "right" }}>
                    ...
                    <span style={{ display: "block", fontSize: 12, fontWeight: 600, color: C.label, marginTop: 2 }}>
                      {t("prop.wasListedAt", {
                        ...
                        d: p.prior_asking_seen_at
                          ? new Date(p.prior_asking_seen_at).toLocaleDateString("en-NZ", {
          ...
          {(() => {
            ...
            // Trade Me — their published figure, filled from their sales export.
            // Context only, and less independent than it looks: for a property
            // that has sold, their number is that sale price carried forward.
            if (p.portal_b_valuation != null)
              tiles.push({
                label: t("prop.srcTrademe"), value: p.portal_b_valuation, provider: true,
                logo: "/logos/trademe.png",
                sub: p.portal_b_valuation_low != null && p.portal_b_valuation_high != null
                  ? `${fmtMoneyShort(p.portal_b_valuation_low)}–${fmtMoneyShort(p.portal_b_valuation_high)}`
                  : undefined,
              });
            else
        ...
        <Card style={{ marginTop: 24 }}>
          ...
          <div
            style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr",
                     gap: isMobile ? 0 : "0 40px", marginTop: 12 }}
          >
            <div>
        ...
        <Card style={{ marginTop: 24 }}>
          ...
          <Note>
            {comps.matched_using?.title && comps.matched_using.title !== "mixed"
              ...
              ? t("prop.mixedTitle")
              : ""}
            {comps.matched_using?.pool === "mixed"
              ? t("prop.mixedPool")
              : comps.matched_using?.pool
              ? t("prop.matchedPool", { pool: comps.matched_using.pool })
              : ""}
      ...
      {/* ───────── SCHOOLS ───────── */}
      {/* Renders nothing when the listing carries no school data. */}
      <SchoolsNearby raw={p.schools_json} />
        ...
        {history && history.points.length > 1 ? (
          <>
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr",
                          gap: isMobile ? 16 : 24, margin: "18px 0 8px" }}>
              <div>
...
function CompRow({ c }: { c: ComparableSale }) {
  ...
  const meta = [
    ...
    c.land_area_m2
      ...
      : null,
    c.type_of_title,
    // A pool changes what a sale is worth, so it belongs on the row rather than
    // leaving the reader to wonder why one sale sits above the others.
    c.has_pool ? "pool" : null,
    method,
    c.sold_date ? fmtDayDate(c.sold_date) : null,
  ]
    .filter(Boolean)
    .join(" · ");
