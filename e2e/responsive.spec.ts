...
const VIEWPORTS = [
  ...
  // A DESKTOP IS NOT COVERED BY "IT WORKS ON A TABLET". Everything above is
  // narrower than a laptop, so until these two rows existed nothing in the
  // suite had ever opened the app at the width most customers use it at — and
  // the faults there are different ones: a fixed max-width that leaves a
  // half-empty screen, a table that only starts scrolling sideways once there
  // is room for every column, a panel that grows past its container.
  { name: "laptop",         width: 1280, height: 800 },   // the common laptop
...
async function overflowing(page: Page, width: number) {
  return page.evaluate((w) => {
    ...
    //
    // globals.css carries `html, body { overflow-x: clip }` as a safety net, so
    // documentElement.scrollWidth is pinned to clientWidth on every page no
    // matter what is inside it — which made this assertion incapable of
    // failing, on any page, since that rule landed. The suite reported green
    // while /admin/users held a table cell reaching 765px on a 390px screen:
    // not scrollable, CLIPPED, and unreachable by the reader.
    //
    // body.scrollWidth still reports the real content width, so that is what
    // this measures now.
    return { scrollWidth: Math.max(doc.scrollWidth, document.body.scrollWidth),
             clientWidth: doc.clientWidth, bad: bad.slice(0, 6) };
  }, width);
}
