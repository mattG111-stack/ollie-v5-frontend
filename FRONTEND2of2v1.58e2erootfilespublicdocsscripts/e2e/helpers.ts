...
/**
 * The id of a listing that actually exists in the seeded batch.
 *
 * Taken from the API, not by clicking through the list UI. The property page is
 * the most layout-heavy screen in the app, and the responsive suite reached it
 * by finding a link on /properties — which defaults to the MAP view, where
 * there are no links. So it called test.skip("no listing link on the page")
 * and reported green, on every viewport, for as long as that has been true.
 *
 * Throws rather than skips. A suite that cannot reach the page it is meant to
 * measure has failed; it has not passed.
 */
export async function firstListingId(): Promise<number> {
  const token = await apiToken(CUSTOMER);
  const ctx = await request.newContext({
    baseURL: "http://127.0.0.1:3100",
    extraHTTPHeaders: { Authorization: `Bearer ${token}` },
  });
  const res = await ctx.get("/api/properties?limit=1");
  if (!res.ok()) throw new Error(`/api/properties failed: ${res.status()}`);
  const body = await res.json();
  await ctx.dispose();
  const rows = Array.isArray(body) ? body : (body.rows ?? body.items ?? body.results ?? []);
  if (!rows.length) throw new Error("no listings in the seeded database — seed_e2e.py is not publishing its batch");
