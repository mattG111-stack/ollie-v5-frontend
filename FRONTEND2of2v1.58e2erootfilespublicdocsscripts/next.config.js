...
const nextConfig = {
  ...
  async rewrites() {
    const raw = (process.env.BACKEND_ORIGIN || "https://backend-production.up.railway.app").trim();
  ...
  // NO REMOTE HOSTS. Every photograph is served through /api/img on our own
  // domain (see the backend's app/media.py), so nothing here should ever name
  // a supplier again — listing this repo's data sources in a config file the
  // browser can fetch was the leak in the first place.
