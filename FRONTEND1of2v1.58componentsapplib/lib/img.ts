...
export function hiRes(url: string | null | undefined, targetCssWidth: number): string {
  ...
  // Our own image path. The source URL is encrypted server-side and never
  // reaches the browser, so none of the rewrites below can see a size token to
  // change — without this every photograph on the site quietly falls back to
  // the ~300px thumbnail the scrape stored. We ask, the proxy applies it to the
  // real URL, which is the only place that still knows what shape it takes.
  if (/^\/api\/img\//.test(url)) {
    return `${url}${url.includes("?") ? "&" : "?"}w=${want}`;
  }
  try {
