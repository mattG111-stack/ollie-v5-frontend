...
export default function BuildVersions() {
  ...
  // Trace one suburb through the real data. "Pick a suburb, nothing matches" has
  // several possible causes and they are indistinguishable from the outside;
  // this prints which one it is rather than leaving it to be guessed.
  const [probe, setProbe] = useState("");

  async function runDiagnostics() {
    setDiagBusy(true);
    try {
      const q = probe.trim() ? `?suburb=${encodeURIComponent(probe.trim())}` : "";
      setDiag(JSON.stringify(await api<unknown>(`/api/admin/diagnostics${q}`), null, 2));
    } catch (e: any) {
    ...
    >
      <div className="flex items-center gap-6 flex-wrap">
        ...
        <input
          ...
          placeholder="trace a suburb (optional)"
          className="ml-auto text-xs px-2.5 py-1.5 border border-line rounded-md w-52 focus:outline-none focus:border-blue"
        />
        <button
          onClick={runDiagnostics}
          disabled={diagBusy}
          className="text-xs px-2.5 py-1.5 border border-line rounded-md hover:border-blue disabled:opacity-50"
        >
