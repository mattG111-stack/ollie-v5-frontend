...
const backendDir = resolve(process.env.E2E_BACKEND_DIR || "../backend");
const testDatabase = join(tmpdir(), `ollie-e2e-${process.pid}.db`);
  ...
  webServer: [
    {
      ...
      // scripts/, not the root. It moved there in v1.54 when the repository
      // root was tidied and this line was not updated, so from that build until
      // v1.83 the web server never started and NOT ONE browser test ran. The
      // failure is silent in the worst way: Playwright reports a config error,
      // and a config error is easy to read as "the harness is being awkward
      // today" rather than as no test coverage at all. Exactly what the comment
      // above about the Chromium path warns against, by a different route.
      // main:app, not app.main:app. The backend stopped being a package in
      // v1.84 — every module sits at the repository root now, because that is
      // the layout the deployment actually has. This line kept the old import
      // path, so the web server would not start and the whole browser suite
      // would go dark again, in the exact way described above. The same word
      // had to change in Procfile, railpack.json and nixpacks.toml; this is the
      // fourth place it is written down and the easiest one to forget.
      command: "python3 scripts/seed_e2e.py && python3 -m uvicorn main:app --host 127.0.0.1 --port 8100",
      cwd: backendDir,
      port: 8100,
